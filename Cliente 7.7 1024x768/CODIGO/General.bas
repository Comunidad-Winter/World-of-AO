...
Sub Addtostatus(RichTextBox As RichTextBox, _
                Text As String, _
                red As Byte, _
                green As Byte, _
                blue As Byte, _
...
'pluto:2.18
    ...
    If UserPeso > UserPesoMax Then
        Call AddtoRichTextBox(frmMain.RecTxt, "Llevas demasiada carga, no puedes moverte.", 150, 150, 150, True, _
                              False, False)
        Exit Sub
...
'*****************************************************************
    ...
    If FPSFast = True Then
        If UserParalizado = True Or UserMeditar Or UserDescansar Then
    ...
    If UserMoving = 0 Then
        If Not UserEstupido Then
            ...
            If GetAsyncKeyState(CustomKeys.BindedKey(eKeyType.mKeyUp)) < 0 Then
                ...
                If FPSFast = True Then
                    If Not LegalPos(UserPos.X, UserPos.Y - 1) Then
            ...
            If GetAsyncKeyState(CustomKeys.BindedKey(eKeyType.mKeyRight)) < 0 Then
                ...
                If FPSFast = True Then
                    If Not LegalPos(UserPos.X + 1, UserPos.Y) Then
        ...
        Else
            Dim kp As Boolean
            kp = (GetAsyncKeyState(CustomKeys.BindedKey(eKeyType.mKeyUp)) < 0) Or GetAsyncKeyState( _
                 CustomKeys.BindedKey(eKeyType.mKeyRight)) < 0 Or GetAsyncKeyState(CustomKeys.BindedKey( _
                                                                                   eKeyType.mKeyDown)) < 0 Or GetAsyncKeyState(CustomKeys.BindedKey(eKeyType.mKeyLeft)) < 0
...
'*************************************************
    ...
    If Not isOpen Then
        ...
        If Dir(App.Path & "\Launcher.exe", vbArchive) = vbNullString Then
            MsgBox _
                    "No se encuentra el archivo de actualización Launcher.exe por favor descarguelo y vuelva a intentar", _
                    vbCritical
            End
        Else
Reintentar:
...
Sub Main()
    ...
    If App.PrevInstance Then
        Call MsgBox( _
             "¡World of AO ya esta corriendo! No es posible correr otra instancia del juego. Haga click en Aceptar para salir.", _
    ...
    If FileExist(App.Path & "\init\ao.dat", vbNormal) Then
        ...
        'If (RenderMod.bUseVideo = False) Then
            'If MsgBox( _
               "¿Desea usar la memoria de video?, en caso de que experimiente bajos FPS puede cambiar esta opción desdel AOSetup que se encuentra en la carpeta del juego.", _
    ...
    CityDesc(1) = _
    "Ullathorpe está establecida en el medio de los grandes bosques de Argentum, es principalmente un pueblo de campesinos y leñadores. Su ubicación hace de Ullathorpe un punto de paso obligado para todos los aventureros ya que se encuentra cerca de los lugares más legendarios de este mundo."
    CityDesc(2) = "Nix es una gran ciudad. Edificada sobre la costa oeste del principal continente de Argentum."
    CityDesc(3) = _
    "Banderbill se encuentra al norte de Ullathorpe y Nix, es una de las ciudades más importantes de todo el imperio."
    ...
    ListaClases(14) = "Pirata"
    'ListaClases(17) = "Ermitaño"
    ListaClases(15) = "Arquero"
    'pluto:2.3
    ...
    AddtoRichTextBox frmCargando.status, "World Of AO", , , , 1
    ...
    AddtoRichTextBox frmCargando.status, "¡Bienvenido al Mundo World of Argentum Online!", , , , 1
    ...
    '----------------
    Call audio.Initialize(DirectX, frmMain.hwnd, App.Path & "\" & Config_Inicio.DirSonidos & "\", App.Path & "\" & _
                                                                                                  Config_Inicio.DirMusica & "\")
    ...
    Do While prgRun
        ...
        If EngineRun Then
            ...
            If frmMain.WindowState <> 1 Then
                ...
                '****** Update screen ******
                Call RenderScreen(UserPos.X - AddtoUserPos.X, UserPos.Y - AddtoUserPos.Y, OffsetCounterX, _
                                  OffsetCounterY)
                'Call DoNightFX
                ...
                'by Lele , cambie esto porque ahora los FPS y la posicion del mapa esta en el MAIN
                ...
                End If
                 ...
                 If PYFLAG Then
                    Call Dialogos.DrawText(260, 760 - (10 * (Party.numMiembros + 1)), "Miembros: " & _
                                                                                      Party.numMiembros, vbWhite, 0)
                    Dim cnmi As Integer
                    Dim cco As Long
                    ...
                    For cnmi = 1 To Party.numMiembros
                        ...
                        If Party.Miembros(cnmi).X = 0 Or Party.Miembros(cnmi).Y = 0 Then
                            Call Dialogos.DrawText(260, 760 - (10 * cnmi), Party.Miembros(cnmi).Nombre & " (" & _
                                                                           Party.Miembros(cnmi).privi & "%)", cco, 0)
                        Else
                            Call Dialogos.DrawText(260, 760 - (10 * cnmi), Party.Miembros(cnmi).Nombre & " (" & _
                                                                           Party.Miembros(cnmi).privi & "%) X: " & Party.Miembros(cnmi).X & " Y: " & _
                                                                           Party.Miembros(cnmi).Y, cco, 0)
                ...
                'pluto:2.12
                If CurMap = 194 Then
                    Call Dialogos.DrawText(445, 260, "El Mejor Luchador es " & UserTorneo2 & " con " & RecordTorneo2 _
                                                     & " Victorias", vbWhite, 0)
                    Call Dialogos.DrawText(495, 275, "Bote Acumulado: " & BoteTorneo2, vbWhite, 0)
                    Call Dialogos.DrawText(445, 290, "El Bote es para quien consiga 10 victorias consecutivas.", _
                                           vbWhite, 0)
...
Function HayAgua(X As Integer, Y As Integer) As Boolean
    ...
    If MapData(X, Y).Graphic(1).GrhIndex >= 1505 And MapData(X, Y).Graphic(1).GrhIndex <= 1520 And MapData(X, _
                                                                                                           Y).Graphic(2).GrhIndex = 0 Then
        HayAgua = True
    Else
        HayAgua = False
