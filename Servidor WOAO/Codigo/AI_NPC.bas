                ...
                If UserList(UI).flags.Muerto = 0 And UserList(UI).flags.Privilegios = 0 Then
                    If Not Criminal(UI) Then
                        Call ChangeNPCChar(ToMap, 0, nPos.Map, NpcIndex, Npclist(NpcIndex).Char.Body, Npclist( _
                                                                                                      NpcIndex).Char.Head, HeadingLoop, 0)
                        Call NpcAtacaUser(NpcIndex, UI)
                        Exit Sub
                    ElseIf Npclist(NpcIndex).flags.AttackedBy = UserList(UI).Name And Not Npclist( _
                           NpcIndex).flags.Follow Then
                        Call ChangeNPCChar(ToMap, 0, nPos.Map, NpcIndex, Npclist(NpcIndex).Char.Body, Npclist( _
                                                                                                      NpcIndex).Char.Head, HeadingLoop, 0)
                        Call NpcAtacaUser(NpcIndex, UI)
                        Exit Sub
            ...
            If UI > 0 Then
                If UserList(UI).flags.Muerto = 0 Then
                    ...
                    If Not UserList(UI).GuildRef Is Nothing Then
                        If Npclist(NpcIndex).Pos.Map = mapa_castillo1 And UserList(UI).GuildRef.IsAllie(castillo1) _
                           Then GoTo aqui2

                        If Npclist(NpcIndex).Pos.Map = mapa_castillo2 And UserList(UI).GuildRef.IsAllie(castillo2) _
                           Then GoTo aqui2

                        If Npclist(NpcIndex).Pos.Map = mapa_castillo3 And UserList(UI).GuildRef.IsAllie(castillo3) _
                           Then GoTo aqui2

                        If Npclist(NpcIndex).Pos.Map = mapa_castillo4 And UserList(UI).GuildRef.IsAllie(castillo4) _
                           Then GoTo aqui2
                    ...
                    If Npclist(NpcIndex).Pos.Map = mapa_castillo1 And UserList(UI).GuildInfo.GuildName = castillo1 _
                       Then GoTo aqui2

                    If Npclist(NpcIndex).Pos.Map = mapa_castillo2 And UserList(UI).GuildInfo.GuildName = castillo2 _
                       Then GoTo aqui2

                    If Npclist(NpcIndex).Pos.Map = mapa_castillo3 And UserList(UI).GuildInfo.GuildName = castillo3 _
                       Then GoTo aqui2

                    If Npclist(NpcIndex).Pos.Map = mapa_castillo4 And UserList(UI).GuildInfo.GuildName = castillo4 _
                       Then GoTo aqui2
...
Private Sub IrUsuarioCercano(ByVal NpcIndex As Integer)
    ...
    For Y = Npclist(NpcIndex).Pos.Y - 10 To Npclist(NpcIndex).Pos.Y + 10
        For X = Npclist(NpcIndex).Pos.X - 13 To Npclist(NpcIndex).Pos.X + 13
            ...
            If X >= MinXBorder And X <= MaxXBorder And Y >= MinYBorder And Y <= MaxYBorder Then
                ...
                If UI > 0 Then
                    ...
                    'pluto:6.0A
                    If UserList(UI).raza = "Vampiro" And (UserList(UI).Char.Body = 9 Or UserList(UI).Char.Body = 260) _
                       Then GoTo aqui
                    ...
                    If UserList(UI).flags.Muerto = 0 And Npclist(NpcIndex).flags.Paralizado = 0 Then
                        ...
                        tHeading = FindDirection(Npclist(NpcIndex).Pos, UserList(MapData(Npclist(NpcIndex).Pos.Map, _
                                                                                         X, Y).Userindex).Pos)

                        'pluto:2.14
                        If Npclist(NpcIndex).flags.PoderEspecial3 > 0 Then
                            If Npclist(NpcIndex).Char.Body <> UserList(UI).Char.Body Or Npclist(NpcIndex).Char.Head _
                               <> UserList(UI).Char.Head Then
                                Npclist(NpcIndex).Char.Body = UserList(UI).Char.Body
                                Npclist(NpcIndex).Char.Head = UserList(UI).Char.Head
                                Call ChangeNPCChar(ToMap, 0, Npclist(NpcIndex).Pos.Map, NpcIndex, Npclist( _
                                                                                                  NpcIndex).Char.Body, Npclist(NpcIndex).Char.Head, tHeading, 0)
...
Private Sub SeguirAgresor(ByVal NpcIndex As Integer)
            ...
            If X >= MinXBorder And X <= MaxXBorder And Y >= MinYBorder And Y <= MaxYBorder Then
                ...
                If UI > 0 Then
                    ...
                    If UserList(UI).Name = Npclist(NpcIndex).flags.AttackedBy Then
                        ...
                        If UserList(UI).flags.Muerto = 0 And UserList(UI).flags.Invisible = 0 And UserList( _
                           UI).flags.AdminInvisible = 0 Then
    ...
    Exit Sub
fallo:
    Call LogError("SEGUIRAGRESOR " & UserList(UI).Name & " -> nº: " & NpcIndex & " nom: " & Npclist(NpcIndex).Name & _
                  " D: " & Err.Description)
            ...
            If X >= MinXBorder And X <= MaxXBorder And Y >= MinYBorder And Y <= MaxYBorder Then
                ...
                If UI > 0 Then
                    If Criminal(UI) And UserList(UI).flags.Privilegios = 0 Then
                        ...
                        If UserList(UI).flags.Muerto = 0 And UserList(UI).flags.Invisible = 0 And UserList( _
                           UI).flags.AdminInvisible = 0 And UserList(UI).flags.Privilegios = 0 Then
        ...
        For X = Npclist(NpcIndex).Pos.X - 13 To Npclist(NpcIndex).Pos.X + 13
            ...
            If X >= MinXBorder And X <= MaxXBorder And Y >= MinYBorder And Y <= MaxYBorder Then
                If Npclist(NpcIndex).Target = 0 And Npclist(NpcIndex).TargetNpc = 0 Then
                    ...
                    If UI > 0 Then
                        If UserList(UI).flags.Muerto = 0 And UserList(UI).flags.Invisible = 0 And UserList( _
                           UI).flags.AdminInvisible = 0 And UI = Npclist(NpcIndex).MaestroUser And Distancia( _
                           Npclist(NpcIndex).Pos, UserList(UI).Pos) > 3 Then
                            tHeading = FindDirection(Npclist(NpcIndex).Pos, UserList(MapData(Npclist( _
                                                                                             NpcIndex).Pos.Map, X, Y).Userindex).Pos)
                            Call MoveNPCChar(NpcIndex, tHeading)
                            Exit Sub
            ...
            If X >= MinXBorder And X <= MaxXBorder And Y >= MinYBorder And Y <= MaxYBorder Then
                ...
                If NI > 0 Then
                    If Npclist(NpcIndex).TargetNpc = NI Then
                        bNoEsta = True
                        tHeading = FindDirection(Npclist(NpcIndex).Pos, Npclist(MapData(Npclist(NpcIndex).Pos.Map, X, _
                                                                                        Y).NpcIndex).Pos)
                        Call MoveNPCChar(NpcIndex, tHeading)
...
Sub HablaPirata(NpcIndex)
    ...
    Call SendData(ToNPCArea, val(NpcIndex), Npclist(NpcIndex).Pos.Map, "!;8°" & n & "°" & Npclist( _
...
ErrorHandler:
    'pluto:2.4.5
    Call LogError("NPCAI " & Npclist(NpcIndex).Name & " " & Npclist(NpcIndex).MaestroUser & " " & Npclist( _
                  NpcIndex).MaestroNpc & " mapa:" & Npclist(NpcIndex).Pos.Map & " x:" & Npclist(NpcIndex).Pos.X & " y:" & _
                  Npclist(NpcIndex).Pos.Y & " Mov:" & Npclist(NpcIndex).Movement & " TargU:" & Npclist(NpcIndex).Target & _
                  " TargN: " & Npclist(NpcIndex).TargetNpc & " N: " & Err.number & " D: " & Err.Description)
    Dim MinPc As npc
    MinPc = Npclist(NpcIndex)
    Call QuitarNPC(NpcIndex)
    Call ReSpawnNpc(MinPc)
...
Function UserNear(ByVal NpcIndex As Integer) As Boolean
    ...
    '#################################################################
    UserNear = Not Int(Distance(Npclist(NpcIndex).Pos.X, Npclist(NpcIndex).Pos.Y, UserList(Npclist( _
                                                                                           NpcIndex).PFINFO.TargetUser).Pos.X, UserList(Npclist(NpcIndex).PFINFO.TargetUser).Pos.Y)) > 1
    Exit Function
fallo:
        ...
        For X = Npclist(NpcIndex).Pos.X - 5 To Npclist(NpcIndex).Pos.X + 5   '5 tiles in every direction
            ...
            If X > MinXBorder And X < MaxXBorder And Y > MinYBorder And Y < MaxYBorder Then
                ...
                If MapData(Npclist(NpcIndex).Pos.Map, X, Y).Userindex > 0 Then
                    'Move towards user
                    tHeading = FindDirection(Npclist(NpcIndex).Pos, UserList(MapData(Npclist(NpcIndex).Pos.Map, X, _
                                                                                     Y).Userindex).Pos)
...
Function PathFindingAI(ByVal NpcIndex As Integer) As Boolean
    ...
    For Y = Npclist(NpcIndex).Pos.Y - 10 To Npclist(NpcIndex).Pos.Y + 10    'Makes a loop that looks at
        For X = Npclist(NpcIndex).Pos.X - 13 To Npclist(NpcIndex).Pos.X + 13   '5 tiles in every direction
            ...
            If X > MinXBorder And X < MaxXBorder And Y > MinYBorder And Y < MaxYBorder Then
                ...
                If MapData(Npclist(NpcIndex).Pos.Map, X, Y).Userindex > 0 Then
                    ...
                    'pluto:2.11
                    If UserList(MapData(Npclist(NpcIndex).Pos.Map, X, Y).Userindex).flags.Privilegios > 0 Or UserList( _
                       MapData(Npclist(NpcIndex).Pos.Map, X, Y).Userindex).flags.Muerto > 0 Then GoTo yop
...
Sub NpcLanzaUnSpell(ByVal NpcIndex As Integer, ByVal Userindex As Integer)
    ...
    If UserList(Userindex).raza = "Vampiro" And (UserList(Userindex).Char.Body = 9 Or UserList(Userindex).Char.Body = _
                                                 260) Then Exit Sub
    ...
    Exit Sub
fallo:
    Call LogError("NPCLANZAUNSPELL " & Npclist(NpcIndex).Name & "-->" & UserList(Userindex).Name & " " & Err.number & _
                  " D: " & Err.Description)
