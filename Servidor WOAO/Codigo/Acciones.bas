...
Sub Accion(ByVal Userindex As Integer, _
    ...
    If InMapBounds(Map, X, Y) Then
        ...
        ElseIf MapData(Map, X + 1, Y).OBJInfo.ObjIndex > 0 Then
            ...
            Call SendData(ToIndex, Userindex, 0, "SELE" & ObjData(MapData(Map, X + 1, Y).OBJInfo.ObjIndex).OBJType & _
                                                 "," & ObjData(MapData(Map, X + 1, Y).OBJInfo.ObjIndex).Name & "," & "OBJ")
        ...
        ElseIf MapData(Map, X + 1, Y + 1).OBJInfo.ObjIndex > 0 Then
            ...
            Call SendData(ToIndex, Userindex, 0, "SELE" & ObjData(MapData(Map, X + 1, Y + _
                                                                                      1).OBJInfo.ObjIndex).OBJType & "," & ObjData(MapData(Map, X + 1, Y + 1).OBJInfo.ObjIndex).Name & _
                                                                                      "," & "OBJ")
        ...
        ElseIf MapData(Map, X, Y + 1).OBJInfo.ObjIndex > 0 Then
            ...
            Call SendData(ToIndex, Userindex, 0, "SELE" & ObjData(MapData(Map, X, Y + 1).OBJInfo.ObjIndex).OBJType & _
                                                 "," & ObjData(MapData(Map, X, Y + 1).OBJInfo.ObjIndex).Name & "," & "OBJ")
...
Sub AccionParaRamita(ByVal Map As Integer, _
    ...
    If UserList(Userindex).Stats.UserSkills(Supervivencia) > 1 And UserList(Userindex).Stats.UserSkills( _
       Supervivencia) < 6 Then
        suerte = 3
    ElseIf UserList(Userindex).Stats.UserSkills(Supervivencia) >= 6 And UserList(Userindex).Stats.UserSkills( _
           Supervivencia) <= 10 Then
        suerte = 2
    ElseIf UserList(Userindex).Stats.UserSkills(Supervivencia) >= 10 And UserList(Userindex).Stats.UserSkills( _
           Supervivencia) Then
        suerte = 1
...
Sub AccionParaForo(ByVal Map As Integer, _
                   ByVal X As Integer, _
                   ByVal Y As Integer, _
                   ByVal Userindex As Integer)
...
Sub AccionParaPuerta(ByVal Map As Integer, _
    ...
    If Not (Distance(UserList(Userindex).Pos.X, UserList(Userindex).Pos.Y, X, Y) > 2) Then
        If ObjData(MapData(Map, X, Y).OBJInfo.ObjIndex).Llave = 1 Then
            If ObjData(MapData(Map, X, Y).OBJInfo.ObjIndex).Cerrada = 1 And Cuentas(Userindex).Llave = ObjData( _
               MapData(Map, X, Y).OBJInfo.ObjIndex).Clave Then
    ...
    If ObjData(MapData(Map, X, Y).OBJInfo.ObjIndex).OBJType = 8 Then
        ...
        If ServerPrimario = 1 Then
            ...
            If MapData(Map, X, Y).OBJInfo.ObjIndex = 78 And Map = 34 Then
                ObjData(78).texto = _
                "Tras advertir el lúgubre aspecto de la vivienda, te diriges hacia la inscripción hallada cerca del umbral y a duras penas logras leer su misiva.                                   Si aquí usted se amparare...             Estertor encontrare...                                                                    Alexstrasza Clamacielos. "
        ...
        If Len(ObjData(MapData(Map, X, Y).OBJInfo.ObjIndex).texto) > 0 Then
            Call SendData2(ToIndex, Userindex, 0, 44, ObjData(MapData(Map, X, Y).OBJInfo.ObjIndex).texto & Chr(176) & _
                                                      ObjData(MapData(Map, X, Y).OBJInfo.ObjIndex).GrhSecundario)
