...
Sub ReSpawnCambioGuardias()
    ...
    For i = 1 To LastNPC
        ...
        If Npclist(i).flags.NPCActive Then
            ...
            If InMapBounds(Npclist(i).Orig.Map, Npclist(i).Orig.X, Npclist(i).Orig.Y) And (Npclist(i).numero = _
                                                                                           Guardias Or Npclist(i).numero = 115) Then
                MinPc = Npclist(i)
                Call QuitarNPC(i)
...
Public Sub Encarcelar(ByVal Userindex As Integer, _
                      ...
                      Optional ByVal GmName As String = "")
    ...
    If GmName = "" Then
        Call SendData(ToIndex, Userindex, 0, "||Has sido encarcelado, deberas permanecer en la carcel " & Minutos & _
                                             " minutos." & "´" & FontTypeNames.FONTTYPE_INFO)
    Else
        Call SendData(ToIndex, Userindex, 0, "||" & GmName & " te ha encarcelado, deberas permanecer en la carcel " & _
                                             Minutos & " minutos." & "´" & FontTypeNames.FONTTYPE_INFO)
