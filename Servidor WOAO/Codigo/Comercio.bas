...
Sub UserCompraObj(ByVal Userindex As Integer, _
    ...
    Exit Sub
fallo:
    Call LogError("USERCOMPRAOBJ " & UserList(Userindex).Name & "npc: " & Npclist(NpcIndex).Name & " obj: " & _
                  ObjIndex & " can: " & Cantidad & " " & Err.number & " D: " & Err.Description)
...
Sub NpcCompraObj(ByVal Userindex As Integer, _
    ...
    If Npclist(NpcIndex).TipoItems <> OBJTYPE_CUALQUIERA And Npclist(NpcIndex).TipoItems <> 888 Then
        ...
        If Npclist(NpcIndex).TipoItems <> ObjData(obji).OBJType Then
            Call SendData(ToIndex, Userindex, 0, "||El npc no esta interesado en comprar ese objeto." & "´" & _
    ...
    Do Until Npclist(NpcIndex).Invent.Object(Slot).ObjIndex = obji And Npclist(NpcIndex).Invent.Object(Slot).Amount + _
    ...
    If Slot <= MAX_INVENTORY_SLOTS Then    'Slot valido
        ...
        Else
            Call SendData(ToIndex, Userindex, 0, "||El npc no puede cargar tantos objetos." & "´" & _
...
Sub NPCVentaItem(ByVal Userindex As Integer, _
    ...
    'pluto:2-3-04
    If (UserList(Userindex).Stats.GLD >= (val * Cantidad) And Npclist(NpcIndex).Comercia = 1) Or (UserList( _
                                                                                                  Userindex).Stats.Puntos >= (val * Cantidad) And Npclist(NpcIndex).Comercia = 2) Then
        ...
        If Npclist(UserList(Userindex).flags.TargetNpc).Invent.Object(i).Amount > 0 Then
            ...
            If Cantidad > Npclist(UserList(Userindex).flags.TargetNpc).Invent.Object(i).Amount Then Cantidad = _
               Npclist(UserList(Userindex).flags.TargetNpc).Invent.Object(i).Amount
    ...
    Else
        ...
        'pluto:2-3-04
        If Npclist(NpcIndex).Comercia = 1 Then Call SendData(ToIndex, Userindex, 0, "||No tenes suficiente Oro." & _
                                                                                    "´" & FontTypeNames.FONTTYPE_INFO) Else Call SendData(ToIndex, Userindex, 0, _
                                                                                                                                          "||No tenes suficientes Puntos de Canje." & "´" & FontTypeNames.FONTTYPE_INFO)
        Exit Sub
...
fallo:
    'pluto:2.10
    Call LogError("NPCVENTAITEM Npc:" & Npclist(NpcIndex).Name & " NpcIndex: " & NpcIndex & " Jug: " & UserList( _
                  Userindex).Name & " " & Err.number & " D: " & Err.Description & "Obj: " & i & "Cant: " & Cantidad & _
                  "TipoNpc: " & Npclist(NpcIndex).NPCtype)

End Sub

Sub NPCCompraItem(ByVal Userindex As Integer, _
...
Sub UpdateVentanaComercio(ByVal Slot As Integer, _
...
Sub EnviarNpcInv(ByVal Userindex As Integer, ByVal NpcIndex As Integer)
    ...
    For i = 1 To MAX_INVENTORY_SLOTS
        ...
        If Npclist(NpcIndex).Invent.Object(i).ObjIndex > 0 Then
            ...
            If val < 1 Then val = 1
            'pluto:6.0A
            Call SendData2(ToIndex, Userindex, 0, 45, Npclist(NpcIndex).Invent.Object(i).ObjIndex & "," & Npclist( _
                                                      NpcIndex).Invent.Object(i).Amount & "," & val)
