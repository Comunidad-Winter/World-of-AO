...
Public Sub ControlaSalidas(ByVal Userindex As Integer, _
    ...
    If InMapBounds(Map, X, Y) Then
        ...
        If MapData(Map, X, Y).TileExit.Map > 0 Then
            ...
            'pluto:2.12
            If UserList(Userindex).Pos.Map = MapaTorneo2 And MapInfo(UserList(Userindex).Pos.Map).NumUsers > 1 And _
               UserList(Userindex).Torneo2 < 10 Then

                Call SendData(ToIndex, Userindex, 0, "||No puedes salir hasta que consigas 10 victorias." & "´" & _
                                                     FontTypeNames.FONTTYPE_INFO)
                'Call ClosestLegalPos(UserList(UserIndex).Pos, nPos, 0)
                'If nPos.X <> 0 And nPos.Y <> 0 Then
                Call WarpUserChar(Userindex, UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList( _
                                                                                                     Userindex).Pos.Y)
            ...
            If UCase$(MapInfo(MapData(Map, X, Y).TileExit.Map).Terreno) = "CASA" And (UserList(Userindex).Stats.GLD < _
                                                                                      30000 Or UserList(Userindex).Invent.ArmourEqpObjIndex = 0 Or EsNewbie(Userindex) Or UserList( _
                                                                                      Userindex).NroMacotas > 0 Or UserList(Userindex).flags.Montura > 0) Then
            ...
            If (MapData(Map, X, Y).TileExit.Map = mapi Or MapData(Map, X, Y).TileExit.Map = 92) And (UserList( _
                                                                                                     Userindex).NroMacotas > 0 Or UserList(Userindex).flags.Montura > 0) Then
                Call SendData(ToIndex, Userindex, 0, "||No puedes acceder a esta sala con mascotas." & "´" & _
                                                     FontTypeNames.FONTTYPE_INFO)
                ' Call ClosestLegalPos(UserList(UserIndex).Pos, nPos, 0)
                'If nPos.X <> 0 And nPos.Y <> 0 Then
            ...
            '-----------------
            'pluto:2-3-04
            If MapInfo(MapData(Map, X, Y).TileExit.Map).StartPos.Map = 178 And MapInfo(MapData(Map, X, _
                                                                                               Y).TileExit.Map).StartPos.Y = 93 And UserList(Userindex).Stats.ELV < 30 Then
                Call SendData(ToIndex, Userindex, 0, "||Necesitas ser Level 30 para acceder a la Pirámide." & "´" & _
                                                     FontTypeNames.FONTTYPE_INFO)
                'Call ClosestLegalPos(UserList(UserIndex).Pos, nPos, 0)
                'If nPos.X <> 0 And nPos.Y <> 0 Then
                Call WarpUserChar(Userindex, UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList( _
            ...
            'If UCase$(MapInfo(MapData(Map, X, Y).TileExit.Map).Terreno) = "CASTILLO" And UserList(Userindex).GuildInfo.GuildName <> "" Then
                ...
                'If Not PuedeEntrarACastillo(Userindex, UserList(Userindex).GuildInfo.GuildName, MapData(Map, X, Y).TileExit.Map) And Not UserList(Userindex).Pos.Map = 166 And Not UserList(Userindex).Pos.Map = 167 And Not UserList(Userindex).Pos.Map = 168 And Not UserList(Userindex).Pos.Map = 169 Then
            ...
            If UCase$(MapInfo(MapData(Map, X, Y).TileExit.Map).Restringir) = "SI" Then
                ...
                If EsNewbie(Userindex) Then
                    If LegalPos(MapData(Map, X, Y).TileExit.Map, MapData(Map, X, Y).TileExit.X, MapData(Map, X, _
                                                                                                        Y).TileExit.Y, PuedeAtravesarAgua(Userindex)) Then

                        If FxFlag Then    '¿FX?
                            Call WarpUserChar(Userindex, MapData(Map, X, Y).TileExit.Map, MapData(Map, X, _
                                                                                                  Y).TileExit.X, MapData(Map, X, Y).TileExit.Y, True)
                        Else
                            Call WarpUserChar(Userindex, MapData(Map, X, Y).TileExit.Map, MapData(Map, X, _
                                                                                                  Y).TileExit.X, MapData(Map, X, Y).TileExit.Y)
                ...
                Else    'No es newbie
                    Call SendData(ToIndex, Userindex, 0, "||Mapa exclusivo para newbies." & "´" & _
                                                         FontTypeNames.FONTTYPE_INFO)
            ...
            Else    'No es un mapa de newbies
                ...
                If LegalPos(MapData(Map, X, Y).TileExit.Map, MapData(Map, X, Y).TileExit.X, MapData(Map, X, _
                    ...
                    If FxFlag Then
                        Call WarpUserChar(Userindex, MapData(Map, X, Y).TileExit.Map, MapData(Map, X, Y).TileExit.X, _
                                          MapData(Map, X, Y).TileExit.Y, True)
                    Else
                        Call WarpUserChar(Userindex, MapData(Map, X, Y).TileExit.Map, MapData(Map, X, Y).TileExit.X, _
                                          MapData(Map, X, Y).TileExit.Y)
...
errhandler:
    Call LogError("Error en ControlaSalidas ->Nom: " & UserList(Userindex).Name & " POS:" & UserList( _
                  Userindex).Pos.Map & " - " & UserList(Userindex).Pos.X & " - " & UserList(Userindex).Pos.Y & " N: " & _
...
q:
            ...
            If UserList(Userindex).Pos.Map = 203 And UserList(Userindex).Faccion.ArmadaReal = 1 Then
            Call SendData(ToIndex, Userindex, 0, "||3°" & Npclist(TempCharIndex).Stats.MinHP & "°" & Npclist( _
                                                         TempCharIndex).Char.CharIndex)
            End If
            
            If UserList(Userindex).Pos.Map = 204 And UserList(Userindex).Faccion.FuerzasCaos = 1 Then
            Call SendData(ToIndex, Userindex, 0, "||3°" & Npclist(TempCharIndex).Stats.MinHP & "°" & Npclist( _
                                                         TempCharIndex).Char.CharIndex)
            End If
