...
Public Sub EfectoLluvia(ByVal Userindex As Integer)
    ...
    If UserList(Userindex).flags.UserLogged Then
        If Intemperie(Userindex) Then
            ...
            modifi = 1 'Porcentaje(UserList(Userindex).Stats.MaxSta, ff)    'ff era un 3
...
Public Sub EfectoMorphUser(ByVal Userindex As Integer)
    ...
    Else
        ...
        '[gau]
        If UserList(Userindex).flags.Morph > 0 Then Call ChangeUserChar(ToMap, 0, UserList(Userindex).Pos.Map, _
                                                                        Userindex, UserList(Userindex).flags.Morph, UserList(Userindex).OrigChar.Head, UserList( _
                                                                                                                                                       Userindex).Char.Heading, UserList(Userindex).Char.WeaponAnim, UserList(Userindex).Char.ShieldAnim, _
                                                                        UserList(Userindex).Char.CascoAnim, UserList(Userindex).Char.Botas, UserList(Userindex).Char.AlasAnim)
