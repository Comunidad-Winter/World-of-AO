...
Sub DarManto(ByVal Userindex As Integer)
    ...
    'Faccion = UserList(Userindex).Faccion.ArmadaReal
    ...
    Call SendData(ToIndex, Userindex, 0, "||Has sido recompensado con el Manto de tu facción!!!" & "´" & _
...
Sub CambiarTrofeo(ByVal Userindex As Integer)
    ...
    Call SendData(ToIndex, Userindex, 0, "||6°Enhorabuena, te has ganado un premio de ganador.!!!°" & str(Npclist( _
...
Sub Desequipar(ByVal Userindex As Integer, ByVal Slot As Byte)
    ...
    Case OBJTYPE_WEAPON
        ...
        If ObjData(UserList(Userindex).Invent.Object(Slot).ObjIndex).objetoespecial = 2 Then
            UserList(Userindex).Stats.UserAtributosBackUP(Fuerza) = UserList(Userindex).Stats.UserAtributosBackUP( _
                                                                    Fuerza) - 5
    ...
    Case OBJTYPE_ARMOUR
        ...
        Case OBJTYPE_ARMADURA
            ...
            Case 2
                UserList(Userindex).Stats.UserAtributosBackUP(Fuerza) = UserList( _
                                                                        Userindex).Stats.UserAtributosBackUP(Fuerza) - 5

            Case 3
                UserList(Userindex).Stats.UserAtributosBackUP(Fuerza) = UserList( _
                                                                        Userindex).Stats.UserAtributosBackUP(Fuerza) - 2

            Case 4
                UserList(Userindex).Stats.UserAtributosBackUP(Fuerza) = UserList( _
                                                                        Userindex).Stats.UserAtributosBackUP(Fuerza) - 3

            Case 5
                UserList(Userindex).Stats.UserAtributosBackUP(Agilidad) = UserList( _
                                                                          Userindex).Stats.UserAtributosBackUP(Agilidad) - 5

            Case 6
                UserList(Userindex).Stats.UserAtributosBackUP(Agilidad) = UserList( _
                                                                          Userindex).Stats.UserAtributosBackUP(Agilidad) - 2

            Case 7
                UserList(Userindex).Stats.UserAtributosBackUP(Agilidad) = UserList( _
                                                                          Userindex).Stats.UserAtributosBackUP(Agilidad) - 3
            ...
            Case 16
                UserList(Userindex).Stats.UserAtributosBackUP(Fuerza) = UserList( _
                                                                        Userindex).Stats.UserAtributosBackUP(Fuerza) - 1
                UserList(Userindex).Stats.UserAtributosBackUP(Agilidad) = UserList( _
                                                                          Userindex).Stats.UserAtributosBackUP(Agilidad) - 1
            ...
            '[GAU] Agregamo UserList(UserIndex).Char.Botas
            Call ChangeUserChar(ToMap, 0, UserList(Userindex).Pos.Map, Userindex, UserList( _
                                ...
                                UserList(Userindex).Char.WeaponAnim, UserList(Userindex).Char.ShieldAnim, UserList( _
                                                                                                          Userindex).Char.CascoAnim, UserList(Userindex).Char.Botas, UserList( _
    ...
    If UserList(Userindex).Stats.MinMAN > UserList(Userindex).Stats.MaxMAN Then UserList(Userindex).Stats.MinMAN = _
       UserList(Userindex).Stats.MaxMAN
...
alli9:
    ...
    Exit Sub
fallo:
    Call LogError("DESEQUIPAR " & Err.number & " D: " & Err.Description & " Nombre: " & UserList(Userindex).Name & _
                  " Obj: " & obj.Name & " Slot: " & Slot)
...
Sub EquiparInvItem(ByVal Userindex As Integer, ByVal Slot As Byte)
    ...
    If UserList(Userindex).flags.Morph > 0 Or UserList(Userindex).flags.Angel > 0 Or UserList( _
       Userindex).flags.Demonio > 0 Then
        Call SendData(ToIndex, Userindex, 0, "||No puedes equipar estando transformado." & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Exit Sub
    ...
    If obj.Newbie = 1 And Not EsNewbie(Userindex) Then
        Call SendData(ToIndex, Userindex, 0, "||Solo los newbies pueden usar este objeto." & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Exit Sub
    ...
    If obj.ObjetoClan <> "" Then
        If UCase$(UserList(Userindex).GuildInfo.GuildName) <> UCase$(obj.ObjetoClan) Then
            Call SendData(ToIndex, Userindex, 0, "||No puedes equipar Ropa de ese Clan" & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
            Exit Sub
    ...
    If obj.RazaEnana = 1 And UserList(Userindex).raza <> "Enano" And UserList(Userindex).raza <> "Gnomo" And UserList( _
       Userindex).raza <> "Goblin" Then
        Call SendData(ToIndex, Userindex, 0, "||Solo los Enanos y Gnomos pueden usar este objeto." & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Exit Sub
    ...
    If obj.Caos > 1 And UserList(Userindex).Faccion.FuerzasCaos = 0 Then
        Call SendData(ToIndex, Userindex, 0, "||Sólo miembros de las Fuerzas del Caos pueden usar este objeto." & "´" _
                                             & FontTypeNames.FONTTYPE_INFO)
        Exit Sub
    ...
    If obj.Real > 0 And UserList(Userindex).Faccion.ArmadaReal <> 1 Then
        Call SendData(ToIndex, Userindex, 0, "||Sólo los miembros de la Armada Real pueden usar este objeto." & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Exit Sub
...
sipuede:
    ...
    If (obj.SubTipo = 6 Or obj.SubTipo = 7) And ObjData(UserList(Userindex).Invent.EscudoEqpObjIndex).SubTipo = 2 And _
       ObjData(UserList(Userindex).Invent.EscudoEqpObjIndex).OBJType = 3 Then
        Call SendData(ToIndex, Userindex, 0, "||No puedes usar Armas de dos Manos con Escudo." & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Exit Sub
...
n:
    ...
    If obj.SubTipo = 2 And obj.OBJType = 3 And (ObjData(UserList(Userindex).Invent.WeaponEqpObjIndex).SubTipo = 6 Or _
                                                ObjData(UserList(Userindex).Invent.WeaponEqpObjIndex).SubTipo = 7) Then
        ...
        Call SendData(ToIndex, Userindex, 0, "||No puedes usar Escudo con Armas de dos Manos." & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Exit Sub
...
n1:
    ...
    Case OBJTYPE_WEAPON
        ...
        If ClasePuedeUsarItem(Userindex, ObjIndex) And FaccionPuedeUsarItem(Userindex, ObjIndex) And _
        ...
        If ClasePuedeUsarItem(Userindex, ObjIndex) And FaccionPuedeUsarItem(Userindex, ObjIndex) And _
           SkillsPuedeUsarItem(Userindex, ObjIndex) Then
            ...
            If ObjData(UserList(Userindex).Invent.AnilloEqpObjIndex).SubTipo = 1 Then
                ...
                If UserList(Userindex).flags.Angel = 0 And UserList(Userindex).flags.Demonio = 0 And UserList( _
                   Userindex).flags.Morph = 0 And MapInfo(UserList(Userindex).Pos.Map).Pk = True Then
                    ...
                    Call SendData2(ToMap, 0, UserList(Userindex).Pos.Map, 16, UserList(Userindex).Char.CharIndex _
                                                                              & ",1")
            ...
            If ObjData(UserList(Userindex).Invent.AnilloEqpObjIndex).SubTipo = 2 Then
                If UserList(Userindex).flags.Morph > 0 Or UserList(Userindex).flags.Angel = 1 Or UserList( _
                   Userindex).flags.Demonio = 1 Or UserList(Userindex).flags.Navegando = 1 Then Exit Sub
                ...
                Call ChangeUserChar(ToMap, 0, UserList(Userindex).Pos.Map, Userindex, val(abody), val(0), _
                ...
                Call SendData(ToPCArea, Userindex, UserList(Userindex).Pos.Map, "CFX" & UserList( _
                                                                                Userindex).Char.CharIndex & "," & Hechizos(43).FXgrh & "," & Hechizos(43).loops)
    ...
    Case OBJTYPE_FLECHAS
        ...
        If ClasePuedeUsarItem(Userindex, UserList(Userindex).Invent.Object(Slot).ObjIndex) And _
           FaccionPuedeUsarItem(Userindex, ObjIndex) And SkillsPuedeUsarItem(Userindex, ObjIndex) Then
    ...
    Case OBJTYPE_ARMOUR
        ...
        If obj.GrhIndex = 25970 And Not UserList(Userindex).flags.LiderAlianza = 1 Then Exit Sub
        If obj.GrhIndex = 26023 And Not UserList(Userindex).flags.LiderHorda = 1 Then Exit Sub
        ...
        Case OBJTYPE_ARMADURA
            ...
            If ClasePuedeUsarItem(Userindex, UserList(Userindex).Invent.Object(Slot).ObjIndex) And _
               ...
               FaccionPuedeUsarItem(Userindex, ObjIndex) And SkillsPuedeUsarItem(Userindex, ObjIndex) _
               Then
                ...
                If UserList(Userindex).Invent.Object(Slot).Equipped Then
                    ...
                    '[GAU] Agregamo UserList(UserIndex).Char.Botas
                    Call ChangeUserChar(ToMap, 0, UserList(Userindex).Pos.Map, Userindex, UserList( _
                                                                                          Userindex).Char.Body, UserList(Userindex).Char.Head, UserList( _
        ...
        Case OBJTYPE_ALAS
            ...
            If ClasePuedeUsarItem(Userindex, UserList(Userindex).Invent.Object(Slot).ObjIndex) Then
                ...
                UserList(Userindex).Char.AlasAnim = obj.AlasAnim
                Call ChangeUserChar(ToMap, 0, UserList(Userindex).Pos.Map, Userindex, UserList( _
                                                                                      Userindex).Char.Body, UserList(Userindex).Char.Head, UserList( _
                                                                                                                                           Userindex).Char.Heading, UserList(Userindex).Char.WeaponAnim, UserList( _
                                                                                                                                                                                                         Userindex).Char.ShieldAnim, UserList(Userindex).Char.CascoAnim, UserList( _
...
Private Function CheckRazaUsaRopa(ByVal Userindex As Integer, _
    ...
    If UserList(Userindex).raza = "Humano" Or UserList(Userindex).raza = "Elfo" Or UserList(Userindex).raza = _
       ...
       Userindex).raza = "Elfo Oscuro" Then
...
Sub TirarTodo(ByVal Userindex As Integer)
    ...
    'PLUTO:6.7 AÑADO MAPA TORNEO TODOSVSTODOS Y SALAS CLAN
        If UserList(Userindex).Pos.Map = 191 Or UserList(Userindex).Pos.Map = 293 Or UserList(Userindex).Pos.Map = 164 Or UserList(Userindex).Pos.Map = 203 Or UserList(Userindex).Pos.Map = 204 Or UserList(Userindex).Pos.Map = 205 Or UserList(Userindex).Pos.Map = 206 Or UserList(Userindex).Pos.Map = 207 Or UserList(Userindex).Pos.Map = 208 Or UserList(Userindex).Pos.Map = _
       MapaTorneo2 Or UCase$(MapInfo(UserList(Userindex).Pos.Map).Terreno) = "CLANATACA" Or UCase$(MapInfo(UserList(Userindex).Pos.Map).Terreno) = "TORNEO" Or UCase$(MapInfo(UserList(Userindex).Pos.Map).Terreno) = "EVENTO" Then
            If Not UserList(Userindex).Pos.Map > 301 Then
            Exit Sub
            End If
       End If
    ...
    If TieneObjetos(1385, 1, Userindex) Then
        ...
        Call SendData(ToIndex, Userindex, 0, "||Sacrificaste un Amuleto de Salvación, en cambio tus items se conservan" & "´" & FontTypeNames.FONTTYPE_INFO)    'Juance!
        Call SendData2(ToPCArea, Userindex, UserList(Userindex).Pos.Map, 22, UserList(Userindex).Char.CharIndex & "," & 33 & "," & 0)
        UserList(Userindex).flags.SeguroRev = True
        Call SendData(ToIndex, Userindex, 0, "||Se activo el seguro de Revivir, para impedir que tus items de caigan al ser revivido." & "´" & FontTypeNames.FONTTYPE_INFO)    'Juance!
        Exit Sub
    End If
