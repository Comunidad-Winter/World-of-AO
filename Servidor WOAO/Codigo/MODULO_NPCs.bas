    ...
    If MinPc.MaestroUser = 0 Then
        ...
        If Userindex > 0 Then    ' Lo mato un usuario?
            ...
            If DifOro > 0 Then
                ...
                Call SendData(ToIndex, Userindex, 0, "|| Has ganado " & MinPc.GiveGLD & " Monedas de Oro." & "´" & _
...
fin:
    ...
    If MinPc.NPCtype = 88 And MinPc.Pos.Map = 92 And Userindex > 0 Then
        If UserList(Userindex).flags.Privilegios = 0 Then
            ...
            DobleExp = 30
            Call SendData(ToAll, Userindex, 0, "|| La energia del Caballero Helado derrotado por " & UserList(Userindex).Name & _
                                               " recorre el mundo Aodrag otorgando a todos los habitantes poderes especiales durante " & DobleExp & " minutos." & _
            ...
            'MsgEntra = "La energía del Caballero Helado derrotado por " & UserList(Userindex).Name & _
    ...
    If MinPc.NPCtype = 145 And MinPc.Pos.Map = 279 And Userindex > 0 Then
        If UserList(Userindex).flags.Privilegios = 0 Then
            ...
            Call SendData(ToAll, Userindex, 0, "|| La Oscuridad del Caballero de la Muerte derrotado por " & UserList(Userindex).Name & _
                                               " recorre el mundo Aodrag otorgando a todos los habitantes poderes especiales durante " & DobleExp & " minutos." & _
                                               "´" & FontTypeNames.FONTTYPE_PARTY)
    ...
    If MinPc.numero = 692 Then
        If Minotauro = UserList(Userindex).Name Then
            ...
            Call SendData(ToAll, Userindex, 0, "|| El Minotauro ha sido asesinado por " & UserList(Userindex).Name & _
                                               " que fué el Personaje que lo liberó, los Dioses le han otorgado los Poderes del Minotauro!!" & _
                                               "´" & FontTypeNames.FONTTYPE_PARTY)
            UserList(Userindex).flags.Minotauro = 1
            Call SendUserStatsFama(Userindex)
        Else
            Call SendData(ToAll, Userindex, 0, "|| El Minotauro ha sido asesinado por " & UserList(Userindex).Name & _
                                               ", este personaje no lo liberó y los Dioses no le conceden los Poderes del Minotauro." & "´" & _
                                               FontTypeNames.FONTTYPE_PARTY)
