...
Public Sub UserDañoNpc(ByVal Userindex As Integer, ByVal NpcIndex As Integer)
    ...
    If UserList(Userindex).raza = "Abisario" Then
...
Public Sub UserDañoUser(ByVal AtacanteIndex As Integer, ByVal VictimaIndex As Integer)
    ...
    'pluto:7.0
    If UserList(AtacanteIndex).raza = "Abisario" Then
        Dim probi As Integer
    ...
    If UserList(AtacanteIndex).Stats.MinHP < Lele And UserList(AtacanteIndex).raza = "Orco" Then
    ...
    If UserList(VictimaIndex).Stats.MinHP < UserList(VictimaIndex).Stats.MaxHP / 3 And UserList(VictimaIndex).raza = "Orco" Then
...
talu:
    ...
    If UserList(VictimIndex).Faccion.ArmadaReal = 1 And UserList(AttackerIndex).Faccion.ArmadaReal = 1 Then
    ...
    Or UserList(AttackerIndex).Pos.Map > 271) And MapInfo(UserList(AttackerIndex).Pos.Map).Terreno <> "EVENTO" Then
    
        Call SendData(ToIndex, AttackerIndex, 0, _
                      "||Los soldados de la alianza no pueden atacarse entre sí." & "´" & _
                      FontTypeNames.FONTTYPE_WARNING)
        PuedeAtacar = False
        Exit Function
    ...
    Or UserList(AttackerIndex).Pos.Map > 271) And MapInfo(UserList(AttackerIndex).Pos.Map).Terreno <> "EVENTO" Then
        ...
        Call SendData(ToIndex, AttackerIndex, 0, _
                      "||Los soldados de la Horda no pueden atacarse entre sí." & "´" & _
                      FontTypeNames.FONTTYPE_WARNING)
        PuedeAtacar = False
        Exit Function
