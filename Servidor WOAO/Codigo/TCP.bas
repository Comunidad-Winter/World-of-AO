...
Sub ConnectNewUser(Userindex As Integer, _
                   ...
                   US10 As String, _
                   US11 As String, _
                   US12 As String, US13 As String, US14 As String, US15 As String, US16 As String, US17 As String, US18 As String, US19 As String, US20 As String, US21 As String, US22 As String, US23 As String, US24 As String, US25 As String, US26 As String, US27 As String, US28 As String, US29 As String, US30 As String, US31 As String, UserEmail As String, Hogar As String, Totalda As Integer, P1 As Byte, P2 As Byte, P3 As Byte, P4 As Byte, P5 As Byte, P6 As Byte)
...
Sub ConnectUser(ByVal Userindex As Integer, _
    ...
    If Not EsDios(Name) And Not EsSemiDios(Name) And SoloGm = True Then
        Call SendData2(ToIndex, Userindex, 0, 43, _
                       "El server en estos momentos está abierto sólo para Gms, estamos comprobando que todo funcione correctamente.")
        Call CloseSocket(Userindex)
        Exit Sub
    ...
    If val(GetVar(Filex, "FLAGS", "BAN")) = 1 Then
        ...
        If rea4 = True Then
            ...
            Call SendData2(ToIndex, Userindex, 0, 107, Name & "fue baneado el dia " & rea2 & " debido a " & rea & _
                                                       ". El personaje ha sido desbaneado")
            Call UnBan(Name)
            Call CloseSocket(Userindex)
        ...
        Else
            Call SendData2(ToIndex, Userindex, 0, 107, Name & " está baneado debido a  " & rea & " desde el dia " & _
                                                       rea2 & "")
            Call CloseSocket(Userindex)
            Exit Sub
    ...
    UserList(Userindex).Stats.PesoMax = (UserList(Userindex).Stats.UserAtributos(1) * 5) + (UserList( _
                                                                                            Userindex).Stats.ELV * 3)
    ...
    Case 185 To 186    'fortaleza
        ...
        If fortaleza <> UserList(Userindex).GuildInfo.GuildName Then
            If Not Criminal(Userindex) Then UserList(Userindex).Pos = Banderbill Else UserList(Userindex).Pos = _
               ciudadcaos
    ...
    Else
        ...
        'pluto:6.5
        If MapData(UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList(Userindex).Pos.Y).Userindex <> 0 _
           Then
            'GetObj (MapData(UserList(UserIndex).Pos.Map, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).UserIndex)
            Call CloseUser(MapData(UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList( _
                                                                                           Userindex).Pos.Y).Userindex)
    ...
    Call SendData2(ToIndex, Userindex, 0, 14, UserList(Userindex).Pos.Map & "," & MapInfo(UserList( _
                                                                                          Userindex).Pos.Map).MapVersion)    'Carga el mapa
    ...
    '[Tite]Nix neutral
    ci = str(MapInfo(1).Dueño) & "," & str(MapInfo(20).Dueño) & "," & str(MapInfo(63).Dueño) & "," & str(MapInfo( _
                                                                                                         81).Dueño) & "," & str(MapInfo(84).Dueño) & "," & str(MapInfo(112).Dueño) & "," & str(MapInfo(151).Dueño) _
                                                                                                         & "," & str(MapInfo(157).Dueño) & "," & str(MapInfo(184).Dueño)
    ...
    If haciendoBK Or haciendoBKPJ Then
        ...
        Call SendData(ToIndex, Userindex, 0, "||Por favor espera algunos segundo, WorldSave esta ejecutandose." & "´" _
    ...
    'pluto:6.8
    If UserList(Userindex).flags.Privilegios = 0 Then MapInfo(UserList(Userindex).Pos.Map).NumUsers = MapInfo( _
       UserList(Userindex).Pos.Map).NumUsers + 1
    ...
    If NumUsers > recordusuarios Then
        Call SendData(ToAll, 0, 0, "||Record de usuarios conectados simultaneamente." & "Hay " & Round(NumUsers) & _
                                   " usuarios." & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If (UserList(Userindex).Faccion.RecompensasCaos > 9 And UserList(Userindex).Faccion.CiudadanosMatados < 800) Or ( _
       UserList(Userindex).Faccion.RecompensasReal > 10 And UserList(Userindex).Faccion.CriminalesMatados < 800 _
       And UserList(Userindex).Faccion.ArmadaReal = 1) Then

        If UserList(Userindex).flags.Privilegios = 0 Then Call LogCasino("Jugador:" & UserList(Userindex).Name & _
                                                                         " mirar recompensas armadas " & "Ip: " & UserList(Userindex).ip)

    End If

    If EsDios(Name) Then
        UserList(Userindex).flags.Privilegios = 3
        Call LogGM(UserList(Userindex).Name, "Se conecto con ip:" & UserList(Userindex).ip & " SE: " & UserList( _
                                             Userindex).Serie)
    ...
    If UserList(Userindex).NroMacotas > 0 Then
        ...
        For i = 1 To MAXMASCOTAS
            ...
            If UserList(Userindex).MascotasType(i) > 0 Then
                UserList(Userindex).MascotasIndex(i) = SpawnNpc(UserList(Userindex).MascotasType(i), UserList( _
                                                                                                     Userindex).Pos, True, True)
...
fff:
    ...
    Exit Sub
fallo:
    Call LogError("connectuser->Nombre: " & Name & " Ip: " & UserList(Userindex).ip & " seña: " & ooo & " " & _
                  Err.Description)
...
Sub ResetBasicUserInfo(ByVal Userindex As Integer)
    ...
    UserList(Userindex).Stats.Elo = 0
    UserList(Userindex).Stats.LibrosUsados = 0
    UserList(Userindex).Stats.Fama = 0
    UserList(Userindex).Stats.exp = 0
...
Sub CloseUser(ByVal Userindex As Integer)
    ...
    If UserList(Userindex).flags.HungerGames = True Then
        ...
        Call SendData(ToMap, 0, 269, "|/Juegos del Hambre" & "> " & UserList(Userindex).Name & _
                                     " se desconectó. El participante fue eliminado del evento.")
    ...
    'Juegos del Hambre Automatico
        ...
        'Juegos del Hambre Automatico
    If UserList(Userindex).flags.BloodGames = True Then
        UserList(Userindex).flags.BloodGames = False
        Call WarpUserChar(Userindex, 34, 50, 50, True)
        Call SendData(ToMap, 0, 269, "|/Blood Castle" & "> " & UserList(Userindex).Name & _
                                     " se desconectó. El participante fue eliminado del evento.")
    ...
    End If
        ...
        If UserList(Userindex).flags.ArenaBattleSlot > 0 Then
            Call RankedTerminate(GetUserRank(Userindex), UserList(Userindex).flags.ArenaBattleSlot, DisconnectUser:=Userindex)
        End If
    ...
    'pluto:2.9.0
    'pluto:2.12 añade torneo2
    'If UserList(Userindex).flags.Paralizado = 1 And UserList(Userindex).flags.Privilegios = 0 Then Call _
    ...
    'pluto:2.11
    If UserList(Userindex).GranPoder > 0 Then UserList(Userindex).GranPoder = 0: UserGranPoder = "": UserList( _
            Userindex).Char.FX = 0
...
alli:
    ...
    UserList(Userindex).Char.FX = 0
    UserList(Userindex).Char.loops = 0
    Call SendData2(ToPCArea, Userindex, UserList(Userindex).Pos.Map, 22, UserList(Userindex).Char.CharIndex & "," & 0 _
                                                                         & "," & 0)
...
End If
    ...
    For i = 1 To MAXMASCOTAS
        ...
        If UserList(Userindex).MascotasIndex(i) > 0 Then
            If Npclist(UserList(Userindex).MascotasIndex(i)).flags.NPCActive Then Call QuitarNPC(UserList( _
                                                                                                 Userindex).MascotasIndex(i))
    ...
    Call SaveUser(Userindex, CharPath & Left$(UCase$(UserList(Userindex).Name), 1) & "\" & UCase$(UserList( _
                                                                                                  Userindex).Name) & ".chr")
    '-------------------------------
    Call ResetUserSlot(Userindex)
...
nup2:
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           38, rdata, 44), ReadField(39, rdata, 44), ReadField(40, rdata, 44), ReadField(41, rdata, _
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         ReadField(45, rdata, 44), ReadField(46, rdata, 44), ReadField(47, rdata, 44), ReadField( _
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       48, rdata, 44), ReadField(49, rdata, 44), ReadField(50, rdata, 44), ReadField(51, rdata, _
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     44), ReadField(52, rdata, 44), ReadField(53, rdata, 44))
            Else
                Call SendData(ToIndex, Userindex, 0, "I1")
                'Call SendData2(ToIndex, UserIndex, 0, 43, "Para jugar a nuestro Server Aodrag (24h Online) bajate el cliente de nuestra web,tenemos torneos automatizados, lucha entre clanes por conquistar Castillos,razas nuevas(orcos y vampiros),gráficos propios con infinidad de armas,escudos,cascos,amuletos.. y muchas más mejoras. Te esperamos en http://www.aodrag.com.ar")
                Call DesconectaCuenta(Userindex)
    ...
    If UCase$(rdata) = "/CONSTRUIR2" Then
        If Not MeterItemEnInventario(Userindex, Arzhnnsz) Then
            ...
            Exit Sub
        End If
        If Not TieneObjetos(1377, 1, Userindex) Or Not TieneObjetos(1387, 3, Userindex) Or Not TieneObjetos(1388, 3, Userindex) Or Not TieneObjetos(1389, 3, Userindex) Then
            Call SendData(ToIndex, Userindex, 0, "||No tienes algunos de los requisitos para construir este objeto." & "´" & FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
        If TieneObjetos(1377, 1, Userindex) And TieneObjetos(1387, 3, Userindex) And TieneObjetos(1388, 3, Userindex) And TieneObjetos(1389, 3, Userindex) Then


            Arzhnnsz.Amount = 1
            Arzhnnsz.ObjIndex = 1379
            Call MeterItemEnInventario(Userindex, Arzhnnsz)
            Call SendData(ToIndex, Userindex, 0, "||Felicitaciones, has construido X item" & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(rdata) = "/CONSTRUIR3" Then
        ...
        If Not TieneObjetos(1379, 1, Userindex) Or Not TieneObjetos(1387, 5, Userindex) Or Not TieneObjetos(1388, 5, Userindex) Or Not TieneObjetos(1389, 5, Userindex) Then
            ...
            Exit Sub
        End If
        If TieneObjetos(1379, 1, Userindex) And TieneObjetos(1387, 5, Userindex) And TieneObjetos(1388, 5, Userindex) And TieneObjetos(1389, 5, Userindex) Then

            Arzhnnsz.Amount = 1
            Arzhnnsz.ObjIndex = 1380
            Call MeterItemEnInventario(Userindex, Arzhnnsz)
            Call SendData(ToIndex, Userindex, 0, "||Felicitaciones, has construido X item" & "´" & FontTypeNames.FONTTYPE_INFO)
