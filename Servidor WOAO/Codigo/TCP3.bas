...
Sub TCP3(ByVal Userindex As Integer, ByVal rdata As String)
    ...
    If UCase$(Left$(rdata, 5)) = "/DEV " Then
        ...
        If UserList(Tindex).PoSum.Map = 0 Then
            Call SendData(ToIndex, Userindex, 0, "||El jugador no tiene guardada su posición." & "´" & _
        ...
        If UserList(Userindex).Pos.Map = 165 And UserList(Userindex).flags.Montura > 0 Then
            Call SendData(ToIndex, Userindex, 0, _
                          "||El jugador no puede ser devuelto porque el mapa no permite mascotas." & "´" & _
                          FontTypeNames.FONTTYPE_INFO)
            Exit Sub

        End If

        Call SendData(ToIndex, Tindex, 0, "||" & UserList(Userindex).Name & " te ha transportado." & "´" & _
                                          FontTypeNames.FONTTYPE_INFO)

        Call WarpUserChar(Tindex, UserList(Tindex).PoSum.Map, UserList(Tindex).PoSum.X, UserList(Tindex).PoSum.Y, True)

        Call LogGM(UserList(Userindex).Name, "/DEV " & UserList(Tindex).Name & " Map:" & UserList(Tindex).PoSum.Map & _
                                             " X:" & UserList(Tindex).PoSum.X & " Y:" & UserList(Tindex).PoSum.Y)
    ...
    If UCase$(Left$(rdata, 3)) = "/CC" Then
        ...
        Call LogGM(UserList(Userindex).Name, "/CC  Map:" & UserList(Userindex).Pos.Map & " X:" & UserList( _
    ...
    If UCase$(Left$(rdata, 3)) = "SPA" Then
        ...
        If val(rdata) > 0 And val(rdata) < UBound(SpawnList) + 1 Then Call SpawnNpc(SpawnList(val(rdata)).NpcIndex, _
                                                                                    UserList(Userindex).Pos, True, False)
    ...
    If UCase$(Left$(rdata, 12)) = "/MAPASEGURO " Then
        ...
        If MapaValido(val(rdata)) Then
            If val(rdata) = MapaSeguro Then
                Call SendData(ToAdmins, Userindex, 0, "||Desactivado Seguro Mapa: " & val(rdata) & "´" & _
                                                      FontTypeNames.FONTTYPE_talk)
                MapaSeguro = 0
            Else
                Call SendData(ToAdmins, Userindex, 0, "||Activado Seguro Mapa: " & val(rdata) & "´" & _
                                                      FontTypeNames.FONTTYPE_talk)
    ...
    If UCase$(Left$(rdata, 7)) = "/TELEP " Then
        ...
        Call SendData(ToIndex, Tindex, 0, "||" & UserList(Userindex).Name & " transportado." & "´" & _
                                          FontTypeNames.FONTTYPE_INFO)
        Call LogGM(UserList(Userindex).Name, "Transporto a " & UserList(Tindex).Name & " hacia " & "Mapa" & Mapa & _
                                             " X:" & X & " Y:" & Y)
        Exit Sub
    ...
    If UCase$(Left$(rdata, 5)) = "/IRA " Then
        ...
        Call LogGM(UserList(Userindex).Name, "/IRA " & UserList(Tindex).Name & " Mapa:" & UserList(Tindex).Pos.Map & _
    ...
    If UCase$(Left$(rdata, 10)) = "/DARPUNTO " Then
        ...
        'If UserList(Tindex).flags.Privilegios > 0 Then
           ' Call SendData(ToIndex, Userindex, 0, "||No puedes dar DragPuntos a un GM." & "´" & FontTypeNames.FONTTYPE_talk)
            'Exit Sub
       ' End If
       Dim Puntos As Integer
       
        If Tindex <= 0 Then
            Call SendData(ToIndex, Userindex, 0, "||Debes seleccionar al Jugador para Darle sus Puntos!" & "´" & FontTypeNames.FONTTYPE_talk)
            Exit Sub
        End If
        Call SendData(ToAll, 0, 0, "||" & UserList(UserList(Userindex).flags.TargetUser).Name & " gano " & rdata & " Puntos de Canje!!!" & "´" & FontTypeNames.FONTTYPE_talk)
        UserList(UserList(Userindex).flags.TargetUser).Stats.Puntos = UserList(UserList(Userindex).flags.TargetUser).Stats.Puntos + rdata
        
        Puntos = UserList(UserList(Userindex).flags.TargetUser).Stats.Puntos
        Call SendData(ToIndex, Tindex, 0, "J5" & Puntos)
        Exit Sub
    End If
    ...
    If UCase(Left(rdata, 3)) = "/CT" Then
        ...
        If MapData(UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList(Userindex).Pos.Y - _
                                                                           1).OBJInfo.ObjIndex > 0 Then
            Exit Sub

        End If

        If MapData(UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList(Userindex).Pos.Y - _
                                                                           1).TileExit.Map > 0 Then
            Exit Sub
        ...
        Call MakeObj(ToMap, 0, UserList(Userindex).Pos.Map, ET, UserList(Userindex).Pos.Map, UserList( _
                                                                                             Userindex).Pos.X, UserList(Userindex).Pos.Y - 1)
        MapData(UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList(Userindex).Pos.Y - 1).TileExit.Map = _
        Mapa
    ...
    If UCase(Left(rdata, 3)) = "/DT" Then
        ...
        If ObjData(MapData(Mapa, X, Y).OBJInfo.ObjIndex).OBJType = OBJTYPE_teleport And MapData(Mapa, X, _
                                                                                                Y).TileExit.Map > 0 Then
    ...
    If UCase$(Left$(rdata, 7)) = "/ECHAR " Then
        ...
        Call SendData(ToAll, 0, 0, "||" & UserList(Userindex).Name & " echo a " & UserList(Tindex).Name & "." & "´" & _
                                   FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 8)) = "/CARCEL " Then
        ...
        If Tindex <= 0 Then
            If FileExist(CharPath & Left$(UCase$(Name), 1) & "\" & UCase$(Name) & ".chr", vbArchive) Then
                Call SendData(ToIndex, Userindex, 0, "||El usuario no esta online pero ha sido Encarcelado." & "´" & _
                                                     FontTypeNames.FONTTYPE_INFO)
                Call WriteVar(CharPath & Left$(UCase$(Name), 1) & "\" & UCase$(Name) & ".chr", "COUNTERS", "Pena", _
                              val(i))
                Call WriteVar(CharPath & Left$(UCase$(Name), 1) & "\" & UCase$(Name) & ".chr", "INIT", "Position", _
                              "66-70-50")
                Exit Sub
            Else
        ...
        If UserList(Tindex).flags.Privilegios > UserList(Userindex).flags.Privilegios Then
            Call SendData(ToIndex, Userindex, 0, "||No podes encarcelar a alguien con jerarquia mayor a la tuya." & _
                                                 "´" & FontTypeNames.FONTTYPE_INFO)
            Exit Sub
...
End If
    ...
    'EZE: TORNEO AUTOMATICO
        ...
        'If UserList(userindex).flags.Privilegios < PlayerType.TournamentManager Then Exit Sub

            If (Torneos > 0 And Torneos < 6) Then
            Call Torneos_Inicia(Userindex, Torneos)
        End If

    'Call LogTorneos("" & UserList(userindex).Name & " - /ATORNEO " & Torneos)
    ...
    If UCase$(Left$(rdata, 5)) = "/SUM " Then
        ...
        If Ayuda.Existe(UserList(Tindex).Name) Or UserList(Userindex).flags.Privilegios > 0 Or UserList( _
           Tindex).Pos.Map = 303 Then
            Call SendData(ToIndex, Tindex, 0, "||" & UserList(Userindex).Name & " te ha transportado." & "´" & _
                                              FontTypeNames.FONTTYPE_INFO)
            'pluto:2.9.0
            Dim aa As Integer
            ...
            Call WarpUserChar(Tindex, UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList( _
                                                                                              Userindex).Pos.Y + aa, True)

            Call LogGM(UserList(Userindex).Name, "/SUM " & UserList(Tindex).Name & " Map:" & UserList( _
    ...
    If UCase$(rdata) = "/TELEPLOC" Then
        Call WarpUserChar(Userindex, UserList(Userindex).flags.TargetMap, UserList(Userindex).flags.TargetX, UserList( _
                                                                                                             Userindex).flags.TargetY, True)
        Call LogGM(UserList(Userindex).Name, "/TELEPLOC a x:" & UserList(Userindex).flags.TargetX & " Y:" & UserList( _
                                             Userindex).flags.TargetY & " Map:" & UserList(Userindex).Pos.Map)
        Exit Sub
    ...
    If UCase$(Left$(rdata, 10)) = "/RECORDHOY" Then
        rdata = Right$(rdata, Len(rdata) - 10)
        Call SendData(ToIndex, Userindex, 0, "||Record de Hoy: " & Round(ReNumUsers) & " Usuarios a las " & HoraHoy & _
                                             "´" & FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||Media de Hoy: " & Round(MediaUsers) & " Usuarios." & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||Record de Ayer: " & Round(AyerReNumUsers) & " Usuarios a las " & _
                                             Horayer & "´" & FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||Media de Ayer: " & Round(AyerMediaUsers) & " Usuarios." & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 9)) = "/AHORCAR " Then
        ...
        If UserList(Tindex).flags.Privilegios > 2 And UserList(Userindex).flags.Privilegios < 3 Then
            ...
            Call SendData(ToGM, Tindex, 0, "|| /El SemiDios " & UserList(Userindex).Name & " ha intentado matar a " & _
                                           UserList(Tindex).Name & "´" & FontTypeNames.FONTTYPE_INFO)
            Exit Sub

        End If

        Call SendData(ToAdmins, 0, 0, "||" & UserList(Userindex).Name & " ahorco a " & UserList(Tindex).Name & "." & _
                                      "´" & FontTypeNames.FONTTYPE_FIGHT)
        Call SendData(ToAll, 0, 0, "||Los dioses han ahorcado a: " & UserList(Tindex).Name & "´" & _
                                   FontTypeNames.FONTTYPE_FIGHT)
        Call UserDie(Tindex)

        Call LogGM(UserList(Userindex).Name, "/AHORCAR " & UserList(Tindex).Name & ": " & UserList(Tindex).Pos.Map & _
                                             ", " & UserList(Tindex).Pos.X & ", " & UserList(Tindex).Pos.Y)
    ...
    If UCase$(Left$(rdata, 9)) = "/REVIVIR " Then
        ...
        Call ChangeUserChar(ToMap, 0, UserList(Tindex).Pos.Map, val(Tindex), UserList(Tindex).Char.Body, UserList( _
                                                                                                         Tindex).OrigChar.Head, UserList(Tindex).Char.Heading, UserList(Tindex).Char.WeaponAnim, UserList( _
                                                                                                                                                                                                 Tindex).Char.ShieldAnim, UserList(Userindex).Char.CascoAnim, UserList(Userindex).Char.Botas, UserList( _
                                                                                                                                                                                                                                                                                              Userindex).Char.AlasAnim)
        '[GAU]
        Call SendUserStatsVida(val(Tindex))
        Call SendData(ToIndex, Tindex, 0, "||" & UserList(Userindex).Name & " te há resucitado." & "´" & _
                                          FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 10)) = "/NUMPARTYS" Then
        ...
        For xpa = 1 To MAXPARTYS
            ...
            If partylist(xpa).numMiembros > 0 Then
                Call SendData(ToIndex, Userindex, 0, "||" & xpa & ": " & UserList(partylist(xpa).lider).Name & " (" & _
                                                     partylist(xpa).numMiembros & ")" & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 12)) = "/NUMMIEMBROS" Then
        ...
        Call SendData(ToIndex, Userindex, 0, "||Miembros: " & partylist(val(rdata)).numMiembros & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        xpa = 0
        ...
        For xpa = 1 To MAXMIEMBROS
            ...
            If partylist(val(rdata)).miembros(xpa).ID <> 0 Then
                Call SendData(ToIndex, Userindex, 0, "||" & UserList(partylist(val(rdata)).miembros(xpa).ID).Name & _
                                                     " (" & partylist(val(rdata)).miembros(xpa).privi & ")" & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 9)) = "/NUMSOLIS" Then
        ...
        rdata = Right$(rdata, Len(rdata) - 10)
        Call SendData(ToIndex, Userindex, 0, "||Solicitudes: " & partylist(val(rdata)).numSolicitudes & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        xpa = 0
        ...
        For xpa = 1 To MAXMIEMBROS
            ...
            If partylist(val(rdata)).Solicitudes(xpa) <> 0 Then
                Call SendData(ToIndex, Userindex, 0, "||" & UserList(partylist(val(rdata)).Solicitudes(xpa)).Name & _
                                                     "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 5)) = "/DEST" Then
        ...
        Dim od3 As Integer
        Dim od2 As Integer
        od3 = MapData(UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList( _
        ...
        rdata = Right$(rdata, Len(rdata) - 5)
        Call EraseObj(ToMap, Userindex, UserList(Userindex).Pos.Map, 10000, UserList(Userindex).Pos.Map, UserList( _
        ...
        If MapaValido(Mapasatu) Then
            ...
            For Y = 1 To 100
                For X = 1 To 100
                    ...
                    If X > 0 And Y > 0 And X < 101 And Y < 101 Then
                        If MapData(Mapasatu, X, Y).OBJInfo.ObjIndex > 0 And MapData(Mapasatu, X, Y).Blocked = 0 Then
                            If ObjData(MapData(Mapasatu, X, Y).OBJInfo.ObjIndex).Agarrable = 0 And ObjData(MapData( _
                                                                                                           Mapasatu, X, Y).OBJInfo.ObjIndex).OBJType = 5 Then
                                Call EraseObj(ToMap, Userindex, Mapasatu, 10000, Mapasatu, X, Y)
                            End If    'blocked
                        End If    'AGARRABLE
                    End If    'x>0
    ...
    If UCase$(Left$(rdata, 14)) = "/LIMPINOSEGURO" Then
        ...
        'Dim Mapasatu As Integer
        Call SendData(ToAll, 0, 0, "||Limpiando Mapas no seguros: Por favor espere ...." & "´" & _
                                   FontTypeNames.FONTTYPE_INFO)
                ...
                If MapInfo(Mapasatu).Pk = True And Mapasatu <> 151 Then
                    ...
                    For Y = 1 To 100
                        For X = 1 To 100
                            ...
                            If X > 0 And Y > 0 And X < 101 And Y < 101 Then
                                If MapData(Mapasatu, X, Y).OBJInfo.ObjIndex > 0 And MapData(Mapasatu, X, Y).Blocked = _
                                   0 Then
    ...
    If UCase$(Left$(rdata, 10)) = "/BUSCAOBJ " Then
        ...
        For Y = 1 To 100
            For X = 1 To 100
                ...
                If X > 0 And Y > 0 And X < 101 And Y < 101 Then If MapData(UserList(Userindex).Pos.Map, X, _
                                                                           Y).OBJInfo.ObjIndex = val(rdata) Then Call SendData(ToIndex, Userindex, 0, "||Cantidad: " & _
                                                                                                                                                      MapData(UserList(Userindex).Pos.Map, X, Y).OBJInfo.Amount & " Posición: " & X & " Y: " & Y & _
    ...
    If UCase$(Left$(rdata, 7)) = "/ALARMA" Then
        ...
        If Alarma = 1 Then Call SendData(ToIndex, Userindex, 0, "||Activada alarma tirada masiva Objetos: " & "´" & _
                                                                FontTypeNames.FONTTYPE_talk)

        If Alarma = 0 Then Call SendData(ToIndex, Userindex, 0, "||Desactivada alarma tirada masiva Objetos: " & "´" _
                                                                & FontTypeNames.FONTTYPE_talk)
        Exit Sub
        ...
        If UserList(Tindex).Alarma = 0 Then
            ...
            For colo = 1 To LastUser
                ...
                If UserList(colo).Alarma > 0 Then
                    Call SendData(ToIndex, Userindex, 0, "||Desactivado Log en " & UserList(colo).Name & "´" & _
                                                         FontTypeNames.FONTTYPE_INFO)
            ...
            Call SendData(ToIndex, Userindex, 0, "||Activado Log en " & UserList(Tindex).Name & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
            Call LogTeclado("-------" & Name & "---------")
            Call SendData2(ToIndex, Tindex, 0, 100)
        Else
            UserList(Tindex).Alarma = 0
            Call SendData(ToIndex, Userindex, 0, "||Desactivado Log en " & UserList(Tindex).Name & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 11)) = "/MAPAANGEL " Then
        ...
        If MapaValido(val(rdata)) Then
            If val(rdata) = MapaAngel Then
                Call SendData(ToAdmins, Userindex, 0, "||Permitido Angeles/Demonios Mapa: " & val(rdata) & "´" & _
                                                      FontTypeNames.FONTTYPE_talk)
                MapaAngel = 0
            Else
                Call SendData(ToAdmins, Userindex, 0, "||Prohibido Angeles/Demonios Mapa: " & val(rdata) & "´" & _
                                                      FontTypeNames.FONTTYPE_talk)
                MapaAngel = val(rdata)
    ...
    If UCase$(rdata) = "/ONLINEMACRO" Then
        ...
        For loopc = 1 To LastUser
            ...
            If (UserList(loopc).Name <> "") And (UCase$(UserList(loopc).clase) = "HERRERO" Or UCase$(UserList( _
                                                                                                     loopc).clase) = "MINERO" Or UCase$(UserList(loopc).clase) = "LEÑADOR" Or UCase$(UserList( _
                                                                                                                                                                                     loopc).clase) = "CARPINTERO" Or UCase$(UserList(loopc).clase) = "ERMITAÑO" Or UCase$(UserList( _
                                                                                                                                                                                                                                                                          loopc).clase) = "DOMADOR" Or UCase$(UserList(loopc).clase) = "PESCADOR") Then
                tStr = tStr & UserList(loopc).Name & ", "
    ...
    If UCase$(Left$(rdata, 8)) = "/PERDON " Then
        ...
        If UserList(Tindex).Faccion.RecibioExpInicialCaos > 0 Then
        UserList(Tindex).Faccion.RecibioExpInicialCaos = 0
        Call SendData(ToIndex, Tindex, 0, "||Fuiste perdonado por un Dios, comportate bien y entrega lo mejor por tu facción." & "´" & _
                    FontTypeNames.FONTTYPE_INFO)
        End If
        
        If UserList(Tindex).Faccion.RecibioExpInicialReal > 0 Then
        UserList(Tindex).Faccion.RecibioExpInicialReal = 0
        Call SendData(ToIndex, Tindex, 0, "||Fuiste perdonado por un Dios, comportate bien y entrega lo mejor por tu facción." & "´" & _
                    FontTypeNames.FONTTYPE_INFO)
        End If
    ...
    If UCase$(Left$(rdata, 6)) = "/SLOT " Then
        ...
        Call SendData(ToIndex, Userindex, 0, "||ConidVálida: " & UserList(NSLOT).ConnIDValida & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        ...
        Call SendData(ToIndex, Userindex, 0, "||Valcode: " & UserList(NSLOT).flags.ValCoDe & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||UserLogged: " & UserList(NSLOT).flags.UserLogged & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||CuentaLogged: " & Cuentas(NSLOT).Logged & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        'pluto:6.6
        Call SendData(ToIndex, Userindex, 0, "||Charindex: " & UserList(NSLOT).Char.CharIndex & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 11)) = "/CHARINDEX " Then
        ...
        Call SendData(ToIndex, Userindex, 0, "||Index en Server: " & UserList(CharList(NuNpc)).Char.CharIndex & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||Name: " & UserList(CharList(NuNpc)).Name & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||Mapa: " & UserList(CharList(NuNpc)).Pos.Map & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||X: " & UserList(CharList(NuNpc)).Pos.X & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||Y: " & UserList(CharList(NuNpc)).Pos.Y & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||Body : " & UserList(CharList(NuNpc)).Char.Body & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||Email : " & UserList(CharList(NuNpc)).Email & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||Hd : " & UserList(CharList(NuNpc)).Serie & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||Valcode: " & UserList(CharList(NuNpc)).flags.ValCoDe & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||UserLogged: " & UserList(CharList(NuNpc)).flags.UserLogged & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||CuentaLogged: " & Cuentas(CharList(NuNpc)).Logged & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 6)) = "/DATOS" Then
        ...
        Call SendData(ToIndex, Userindex, 0, "||Posicion: " & UserList(Tindex).Pos.Map & "-" & UserList(Tindex).Pos.X _
                                             & "-" & UserList(Tindex).Pos.Y & "´" & FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||Conid: " & UserList(Tindex).ConnID & "´" & FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||ConidVálida: " & UserList(Tindex).ConnIDValida & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        ...
        Call SendData(ToIndex, Userindex, 0, "||Valcode: " & UserList(Tindex).flags.ValCoDe & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||UserLogged: " & UserList(Tindex).flags.UserLogged & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||CuentaLogged: " & Cuentas(Tindex).Logged & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Exit Sub
    ...
    If UCase$(Left$(rdata, 9)) = "/USERMAP " Then
        ...
        tStr = Left$(tStr, Len(tStr) - 2)
        Call SendData(ToIndex, Userindex, 0, "||" & tStr & "´" & FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||Número de Usuarios en Mapa " & Mapa & ": " & MapInfo(Mapa).NumUsers & _
                                             "´" & FontTypeNames.FONTTYPE_INFO)
        Exit Sub
    ...
    If UCase$(Left$(rdata, 7)) = "/CONEX " Then
        ...
        Call SendData(ToAll, 0, 0, "||" & UserList(Userindex).Name & " cerró conexión a " & UserList(Tindex).Name & _
                                   "." & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 11)) = "/CUENTABAN " Then
        ...
        If accindex = 0 Then
            ...
            Call WriteVar(App.Path & "\logs\" & "BanAccountDetail.dat", ReadField(2, rdata, Asc("#")), "BannedBy", _
            ...
            Call SendData(ToIndex, Userindex, 0, "||La cuenta se ha baneado directamente a la ficha." & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
        Else
    ...
    If UCase$(Left$(rdata, 13)) = "/UNBANCUENTA " Then
        ...
        If accindex = 0 Then
            ...
            Call SendData(ToIndex, Userindex, 0, "||La cuenta se ha Desbaneado directamente a la ficha." & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 7)) = "/PJBAN " Then
        ...
        If UserList(Tindex).flags.Privilegios > UserList(Userindex).flags.Privilegios Then
            Call SendData(ToIndex, Userindex, 0, "||No podes banear a al alguien de mayor jerarquia." & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        ...
        Call SendData(ToAdmins, 0, 0, "||" & UserList(Userindex).Name & " echo a " & UserList(Tindex).Name & "." & _
                                      "´" & FontTypeNames.FONTTYPE_FIGHT)
        Call SendData(ToAdmins, 0, 0, "||" & UserList(Userindex).Name & " Banned a " & UserList(Tindex).Name & "." & _
                                      "´" & FontTypeNames.FONTTYPE_FIGHT)
        Call SendData(ToAll, 0, 0, "||Los dioses han desterrado a: " & UserList(Tindex).Name & "´" & _
                                   FontTypeNames.FONTTYPE_FIGHT)
        ...
        If UserList(Tindex).flags.Privilegios > 0 Then
            ...
            Call SendData(ToAdmins, 0, 0, "||" & UserList(Userindex).Name & _
                                          " banned by the server por bannear un Administrador." & "´" & FontTypeNames.FONTTYPE_FIGHT)
            Exit Sub
    ...
    If UCase$(Left$(rdata, 9)) = "/BLOQUEO " Then
        ...
        If Tindex <= 0 Then    'no ta online
            ...
            If PersonajeExiste(namebloqueo) Then
                ...
                If Not FileExist(App.Path & "\Bloqueos\" & mira2 & ".lol", vbArchive) Then
                    ...
                    Call SendData(ToIndex, Userindex, 0, "||" & rdata & " Ha sido Bloqueado." & "´" & _
                                                         FontTypeNames.FONTTYPE_talk)
                    Call LogGM(UserList(Userindex).Name, "Bloqueo a " & namebloqueo)
                    '@Nati: Comienzo Crea el LOG
                    Call WriteVar(App.Path & "\logs\" & "BanPCDetail.dat", namebloqueo, "BannedBy", UserList( _
                                                                                                    Userindex).Name)
                ...
                Else
                    ...
                    Call SendData(ToIndex, Userindex, 0, "||" & rdata & " Ha sido DesBloqueado." & "´" & _
                                                         FontTypeNames.FONTTYPE_talk)
        ...
        If Not FileExist(App.Path & "\Bloqueos\" & UserList(Tindex).Serie & ".lol", vbArchive) Then
            ...
            Call WriteVar(App.Path & "\Bloqueos\" & UserList(Tindex).Serie & ".lol", "INIT", "NOMBRE", UserList( _
                                                                                                       Tindex).Name)
    ...
    If UCase$(Left$(rdata, 12)) = "/MACBLOQUEO " Then
        ...
        If Tindex <= 0 Then    'no ta online
            ...
            If PersonajeExiste(rdata) Then
                ...
                If Not FileExist(App.Path & "\MacPluto\" & mira2 & ".lol", vbArchive) Then
                    ...
                    Call SendData(ToIndex, Userindex, 0, "||" & rdata & " Ha sido Mac-Bloqueado." & "´" & _
                                                         FontTypeNames.FONTTYPE_talk)
                    Call LogGM(UserList(Userindex).Name, "Mac-Bloqueo a " & rdata)
                Else
                    Kill (App.Path & "\MacPluto\" & mira2 & ".lol")
                    Call SendData(ToIndex, Userindex, 0, "||" & rdata & " Ha sido Mac-DesBloqueado." & "´" & _
                                                         FontTypeNames.FONTTYPE_talk)
        ...
        If Not FileExist(App.Path & "\MacPluto\" & UserList(Tindex).MacPluto & ".lol", vbArchive) Then
            ...
            Call WriteVar(App.Path & "\MacPluto\" & UserList(Tindex).MacPluto & ".lol", "INIT", "NOMBRE", UserList( _
                                                                                                          Tindex).Name)
        Else
            Call SendData(ToIndex, Userindex, 0, "||" & rdata & " Ha sido Mac-DesBloqueado." & "´" & _
                                                 FontTypeNames.FONTTYPE_talk)
    ...
    If UCase(rdata) = "/CANCELARHUNGER" Then
        Call HungerGames_Cancela
        Exit Sub

    End If
    
        'Juegos del Hambre Automatico
    If UCase(rdata) = "/CANCELARBLOOD" Then
...
End If
    ...
    If UCase$(Left$(rdata, 12)) = "/AUTOHUNGER " Then
        rdata = Right$(rdata, Len(rdata) - 12)
        CantHungerGames = CInt(rdata)

        If (CantHungerGames > 0 And CantHungerGames < 50) Then Call HungerGames_Comienza(CantHungerGames)

    End If
    
        If UCase$(Left$(rdata, 11)) = "/AUTOBLOOD " Then
        rdata = Right$(rdata, Len(rdata) - 11)
        CantBloodGames = CInt(rdata)

        If (CantBloodGames > 0 And CantBloodGames < 50) Then Call BloodGames_Comienza(CantBloodGames)
    ...
    If UCase$(Left$(rdata, 5)) = "/ACC " Then
        ...
        Call LogGM(UserList(Userindex).Name, "/ACC Bicho:" & val(rdata) & " Map:" & UserList(Userindex).Pos.Map & _
                                             " X:" & UserList(Userindex).Pos.X & " Y:" & UserList(Userindex).Pos.Y)
        Exit Sub
    ...
    If UCase$(Left$(rdata, 7)) = "/CLAVE " Then
        ...
        If (Len(ReadField(2, rdata, 44)) < 7) Then
            Call SendData(ToIndex, Userindex, 0, "||Contraseña demasiado corta, escriba una mas grande" & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        ...
        If CuentaExiste(nickx) Then
            ...
            Call SendData(ToIndex, Userindex, 0, "||La nueva clave es: " & ReadField(2, rdata, 44) & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 9)) = "/BUSCAIP " Then
        ...
        For n = 1 To MaxUsers
            ...
            If UserList(n).ip = UserList(Tindex).ip Then Call SendData(ToIndex, Userindex, 0, "|| " & UserList( _
                                                                                              n).Name & "´" & FontTypeNames.FONTTYPE_INFO)
        Next
        Exit Sub
    ...
    If UCase$(Left$(rdata, 12)) = "/BUSCASERIE " Then
        ...
        For n = 1 To MaxUsers
            ...
            If UserList(n).Serie = rdata Then Call SendData(ToIndex, Userindex, 0, "|| " & UserList(n).Name & "´" & _
                                                                                   FontTypeNames.FONTTYPE_INFO)
        Next
        Exit Sub
    ...
    If UCase$(Left$(rdata, 10)) = "/BUSCAMAC " Then
        ...
        For n = 1 To MaxUsers
            ...
            If UserList(n).MacPluto = rdata Then Call SendData(ToIndex, Userindex, 0, "|| " & UserList(n).Name & "´" _
                                                                                      & FontTypeNames.FONTTYPE_INFO)
        Next
        Exit Sub
    ...
    If UCase$(Left$(rdata, 10)) = "/PJCUENTA " Then
        ...
        If CuentaExiste(rdata) Then
            ...
            For n = 1 To GetVar(archiv, "DATOS", "NumPjs")
                Call SendData(ToIndex, Userindex, 0, "||Pj: " & GetVar(archiv, "PERSONAJES", "PJ" & n) & "´" & _
                                                     FontTypeNames.FONTTYPE_INFO)
            Next n
    ...
    If UCase$(Left$(rdata, 7)) = "/MOVER " Then
        ...
        If PersonajeExiste(rdata) Then
            Call SendData(ToIndex, Userindex, 0, "|| El pj ha sido transportado a nix." & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 7)) = "/RMSG2 " Then
        ...
        If rdata <> "" Then
            Call SendData(ToMap, 0, UserList(Userindex).Pos.Map, "||" & UserList(Userindex).Name & ": " & rdata & "´" _
                                                                 & FontTypeNames.FONTTYPE_talk)
    ...
    If UCase$(Left$(rdata, 9)) = "/MENSAJE " Then
        ...
        If rdata <> "" Then MsgEntra = rdata: Call SendData(ToIndex, Userindex, 0, "||Mensaje de Entrada Activado: " _
                                                                                   & rdata & "´" & FontTypeNames.FONTTYPE_INFO)

        If UCase$(rdata) = "NINGUNO" Then MsgEntra = "": Call SendData(ToIndex, Userindex, 0, _
                                                                       "||Mensaje de Entrada Desactivado." & "´" & FontTypeNames.FONTTYPE_INFO)
        Exit Sub
    ...
    If UCase$(Left$(rdata, 8)) = "/IPNICK " Then
        ...
        If Tindex > 0 Then
            Call SendData(ToIndex, Userindex, 0, "||El ip de " & rdata & " es " & UserList(Tindex).ip & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
        Else
    ...
    If UCase$(Left$(rdata, 8)) = "/NICKIP " Then
        ...
        If Tindex > 0 Then
            Call SendData(ToIndex, Userindex, 0, "||El nick del ip " & rdata & " es " & UserList(Tindex).Name & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
        Else
            Call SendData(ToIndex, Userindex, 0, "||No existe" & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(rdata) = "/MATA2" Then
        ...
        Else
            Call LogGM(UserList(Userindex).Name, "Intento a un NO-GUARDIAN /MATA2 " & Npclist(UserList( _
                                                                                              Userindex).flags.TargetNpc).Name)
    ...
    If UCase$(Left$(rdata, 5)) = "/BLOQ" Then
        ...
        If UserList(Userindex).flags.Privilegios < 2 Then    'PRIVILEGIOS >
            If UserList(Userindex).Pos.Map = 303 Then    'MAPA 303 >
                If MapData(UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList(Userindex).Pos.Y).Blocked _
                   = 0 Then
                    MapData(UserList(UserIndeax).Pos.Map, UserList(Userindex).Pos.X, UserList( _
                                                                                     Userindex).Pos.Y).Blocked = 1
                    Call Bloquear(ToMap, Userindex, UserList(Userindex).Pos.Map, UserList(Userindex).Pos.Map, _
                                  UserList(Userindex).Pos.X, UserList(Userindex).Pos.Y, 1)
                Else
                    MapData(UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList( _
                                                                                    Userindex).Pos.Y).Blocked = 0
                    Call Bloquear(ToMap, Userindex, UserList(Userindex).Pos.Map, UserList(Userindex).Pos.Map, _
                                  UserList(Userindex).Pos.X, UserList(Userindex).Pos.Y, 0)
        ...
        Else    'PRIVILEGIOS <>
            ...
            If MapData(UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList(Userindex).Pos.Y).Blocked = 0 _
               Then
                MapData(UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList(Userindex).Pos.Y).Blocked = 1
                Call Bloquear(ToMap, Userindex, UserList(Userindex).Pos.Map, UserList(Userindex).Pos.Map, UserList( _
                                                                                                          Userindex).Pos.X, UserList(Userindex).Pos.Y, 1)
            Else
                MapData(UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList(Userindex).Pos.Y).Blocked = 0
                Call Bloquear(ToMap, Userindex, UserList(Userindex).Pos.Map, UserList(Userindex).Pos.Map, UserList( _
                                                                                                          Userindex).Pos.X, UserList(Userindex).Pos.Y, 0)
    ...
    If UCase$(Left$(rdata, 7)) = "/DONDE " Then
        ...
        If UserList(Tindex).flags.Privilegios > 2 And UserList(Userindex).flags.Privilegios < 3 Then
            ...
            Call SendData(ToGM, Tindex, 0, "|| /DONDE del SemiDios " & UserList(Userindex).Name & " sobre el Dios " & _
                                           UserList(Tindex).Name & "´" & FontTypeNames.FONTTYPE_INFO)
            Exit Sub

        End If

        Call SendData(ToIndex, Userindex, 0, "||Ubicacion  " & UserList(Tindex).Name & ": " & UserList( _
                                             Tindex).Pos.Map & ", " & UserList(Tindex).Pos.X & ", " & UserList(Tindex).Pos.Y & "." & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        'pluto:2-3-04
        Call LogGM(UserList(Userindex).Name, "/Donde " & UserList(Tindex).Name & ": " & UserList(Tindex).Pos.Map & _
                                             ", " & UserList(Tindex).Pos.X & ", " & UserList(Tindex).Pos.Y)
    ...
    If UCase$(Left$(rdata, 10)) = "/FOTOINVI " Then
        ...
        If Tindex <= 0 Then Call SendData(ToIndex, Userindex, 0, "||No está online" & "´" & _
                                                                 FontTypeNames.FONTTYPE_INFO): Exit Sub
    ...
    If UCase$(Left$(rdata, 10)) = "/DARPODER " Then
        ...
        Call SendData(ToIndex, Userindex, 0, "||Gran Poder pasado a: " & UserGranPoder & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 9)) = "/JODETE1 " Then
        ...
        Call SendData(ToIndex, Tindex, 0, "||Ventanitas de aviso activadas sobre el user" & "´" & _
                                          FontTypeNames.FONTTYPE_talk)
    ...
    If UCase$(Left$(rdata, 7)) = "/FICHA " Then
        ...
        If PersonajeExiste(rdata) Then
            ...
            Call SendData(ToIndex, Userindex, 0, "||Su email de creación: " & GetVar(archiv, "CONTACTO", "Email") & _
                                                 "´" & FontTypeNames.FONTTYPE_INFO)

            Call SendData(ToIndex, Userindex, 0, "||Su última Ip es:" & GetVar(archiv, "INIT", "LastIP") & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
            Call SendData(ToIndex, Userindex, 0, "||Su última HD es:" & GetVar(archiv, "INIT", "LastSerie") & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
            Call SendData(ToIndex, Userindex, 0, "||Su última MAC es:" & GetVar(archiv, "INIT", "LastMac") & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)

            Call SendData(ToIndex, Userindex, 0, "||Clase:" & GetVar(archiv, "INIT", "Clase") & " " & GetVar(archiv, _
                                                                                                             "STATS", "Elv") & "´" & FontTypeNames.FONTTYPE_INFO)
            Call SendData(ToIndex, Userindex, 0, "||Oro:" & GetVar(archiv, "STATS", "Gld") & " Banco: " & GetVar( _
                                                 archiv, "STATS", "Banco") & "´" & FontTypeNames.FONTTYPE_INFO)
            Call SendData(ToIndex, Userindex, 0, "||Puntos de Canje:" & GetVar(archiv, "STATS", "Puntos") & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
            'pluto:2.17
            Call SendData(ToIndex, Userindex, 0, "||Remort:" & GetVar(archiv, "STATS", "Remort") & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
            ...
            If Tindex <= 0 Then Call SendData(ToIndex, Userindex, 0, "|| No está Online. " & "´" & _
                                                                     FontTypeNames.FONTTYPE_INFO): GoTo yap
            'pluto:2.10 email cuenta
            Call SendData(ToIndex, Userindex, 0, "||Su email actual es: " & Cuentas(Tindex).mail & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)    'GetVar(archiv, "CONTACTO", "Email") & FONTTYPENAMES.FONTTYPE_INFO)

            Call SendData(ToIndex, Userindex, 0, "||Fuerza: " & UserList(Tindex).Stats.UserAtributosBackUP(1) & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
            Call SendData(ToIndex, Userindex, 0, "||Agilid: " & UserList(Tindex).Stats.UserAtributosBackUP(2) & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
            Call SendData(ToIndex, Userindex, 0, "||Inteli: " & UserList(Tindex).Stats.UserAtributosBackUP(3) & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
            Call SendData(ToIndex, Userindex, 0, "||Carism: " & UserList(Tindex).Stats.UserAtributosBackUP(4) & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
            Call SendData(ToIndex, Userindex, 0, "||Consti: " & UserList(Tindex).Stats.UserAtributosBackUP(5) & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
yap:
        Else
            Call SendData(ToIndex, Userindex, 0, "||El usuario no existe" & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(rdata) = "/CONSUMO" Then
        ...
        Call SendData(ToIndex, Userindex, 0, "|| Recibidos: " & TotalBytesRecibidos & " kb = " & Round( _
                                             TotalBytesRecibidos / 1048576, 3) & " Gb." & "´" & FontTypeNames.FONTTYPE_INFO)
        Dim medihora As Long
        medihora = Round((TotalBytesRecibidos / MinutosOnline) * 60, 3)
        Call SendData(ToIndex, Userindex, 0, "|| Media por Hora: " & medihora & " kb = " & Round(medihora / 1048576, _
                                                                                                 3) & " Gb." & "´" & FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "|| Media por Día: " & medihora * 24 & " kb = " & Round((medihora * 24) _
                                                                                                     / 1048576, 3) & " Gb." & "´" & FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "|| Media por Mes: " & medihora * 24 * 30 & " kb = " & Round((medihora * _
                                                                                                           24 * 30) / 1048576, 3) & " Gb." & "´" & FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "|| ------------------- " & "´" & FontTypeNames.FONTTYPE_INFO)
        medihora = Round((TotalBytesEnviados / MinutosOnline) * 60, 3)
        Call SendData(ToIndex, Userindex, 0, "|| Enviados: " & TotalBytesEnviados & " kb = " & Round( _
                                             TotalBytesEnviados / 1048576, 3) & " Gb." & "´" & FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "|| Media por Hora: " & medihora & " kb = " & Round(medihora / 1048576, _
                                                                                                 3) & " Gb." & "´" & FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "|| Media por Día: " & medihora * 24 & " kb = " & Round((medihora * 24) _
                                                                                                     / 1048576, 3) & " Gb." & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase(Left(rdata, 12)) = "/ONLINEMAPA " Then
        ...
        If tStr = "" Then
            Call SendData(ToIndex, Userindex, 0, "||Usuarios en Mapa " & val(rdata) & ": " & ab & "´" & _
                                                 FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        ...
        tStr = Left$(tStr, Len(tStr) - 2)
        Call SendData(ToIndex, Userindex, 0, "||" & tStr & "´" & FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||Usarios en Mapa " & val(rdata) & ": " & ab & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        'pluto:6.7
    ...
    If UCase(Left(rdata, 7)) = "/WCHAR " Then
        ...
        Call SendData(ToIndex, Userindex, 0, "||Variable escrita. (" & Arg1 & "):" & Arg2 & ":" & Arg3 & ":=" & Arg4 _
                                             & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase(Left(rdata, 7)) = "/RCHAR " Then
        ...
        If Not PersonajeExiste(Arg1) Then Exit Sub
        Call SendData(ToIndex, Userindex, 0, "||(" & Arg1 & "):" & Arg2 & ":" & Arg3 & ":=" & GetVar(CharPath & Left$( _
                                                                                                     Arg1, 1) & "\" & Arg1 & ".chr", Arg2, Arg3) & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase$(Left$(rdata, 10)) = "/RACCOUNT " Then
        ...
        Call SendData(ToIndex, Userindex, 0, "||(" & Arg1 & "):" & Arg2 & ":" & Arg3 & ":=" & GetVar(AccPath & "\" & _
                                                                                                     Arg1 & ".acc", Arg2, Arg3) & "´" & FontTypeNames.FONTTYPE_INFO)
    ...
    If UCase(Left(rdata, 6)) = "/VIDA " Then
        ...
        Call LogGM(UserList(Userindex).Name, "/VIDA " & rdata & " : " & Npclist(UserList( _
                                                                                Userindex).flags.TargetNpc).Name)
        Exit Sub
    ...
    If UCase(Left(rdata, 5)) = "/EXP " Then
        ...
        Call LogGM(UserList(Userindex).Name, "/Exp " & rdata & " : " & Npclist(UserList( _
                                                                               Userindex).flags.TargetNpc).Name)
        Exit Sub
    ...
    If UCase(Left(rdata, 5)) = "/DMG " Then
        ...
        Call SendData(ToIndex, Tindex, 0, "||" & CartelDMG & " te ha quitado " & DMG & " puntos de vida." & "´" & _
                                          FontTypeNames.FONTTYPE_FIGHT)
    ...
    If UCase$(rdata) = "/MASSKILL" Then
        ...
        For Y = UserList(Userindex).Pos.Y - MinYBorder + 1 To UserList(Userindex).Pos.Y + MinYBorder - 1
            For X = UserList(Userindex).Pos.X - MinXBorder + 1 To UserList(Userindex).Pos.X + MinXBorder - 1
                ...
                If X > 0 And Y > 0 And X < 101 And Y < 101 Then If MapData(UserList(Userindex).Pos.Map, X, _
                                                                           Y).NpcIndex > 0 Then Call QuitarNPC(MapData(UserList(Userindex).Pos.Map, X, Y).NpcIndex)
            Next X
        Next Y
    ...
    If UCase$(Left$(rdata, 12)) = "/SACARPUNTO " Then
        ...
        Tindex = UserList(Userindex).flags.TargetUser
        If Tindex <= 0 Then
            Call SendData(ToIndex, Userindex, 0, "||Debes seleccionar al Jugador para Sacarle sus Puntos!" & "´" & FontTypeNames.FONTTYPE_talk)
            Exit Sub
        End If
        If UserList(Tindex).Stats.Puntos < rdata Then
            Call SendData(ToIndex, Userindex, 0, "||No puedes Sacar esa Cantidad de Puntos, Genera Variable Muerta!" & "´" & FontTypeNames.FONTTYPE_talk)
            Exit Sub
        End If
        Call SendData(ToAll, 0, 0, "||" & UserList(UserList(Userindex).flags.TargetUser).Name & " se le restaron " & rdata & " Puntos de Canje!!!" & "´" & FontTypeNames.FONTTYPE_talk)
        UserList(UserList(Userindex).flags.TargetUser).Stats.Puntos = UserList(UserList(Userindex).flags.TargetUser).Stats.Puntos - rdata
    ...
    If UCase$(Left$(rdata, 5)) = "/MOD " Then
        ...
        Case "ORO"
            ...
            Else
                Call SendData(ToIndex, Userindex, 0, _
                              "||No esta permitido utilizar valores mayores a 95000. Su comando ha quedado en los logs del juego." _
                              & "´" & FontTypeNames.FONTTYPE_INFO)
                Exit Sub
        ...
        Case "OBJETO"
            ...
            If val(Arg2) > NumObjDatas Then
                Call SendData(ToIndex, Userindex, 0, "||No hay tantos objetos." & "´" & _
                                                     FontTypeNames.FONTTYPE_WARNING)
                Exit Sub

            End If

            If ObjData(val(Arg2)).OBJType <> 40 And ObjData(val(Arg2)).OBJType <> OBJTYPE_USEONCE And ObjData(val( _
                                                                                                              Arg2)).OBJType <> OBJTYPE_WEAPON And ObjData(val(Arg2)).OBJType <> OBJTYPE_ARMOUR And ObjData( _
                                                                                                              val(Arg2)).OBJType <> OBJTYPE_POCIONES And ObjData(val(Arg2)).OBJType <> OBJTYPE_BEBIDA And _
                                                                                                              ObjData(val(Arg2)).OBJType <> OBJTYPE_LEÑA And ObjData(val(Arg2)).OBJType <> _
                                                                                                              OBJTYPE_HERRAMIENTAS And ObjData(val(Arg2)).OBJType <> OBJTYPE_PERGAMINOS And ObjData(val( _
                                                                                                                                                                                                    Arg2)).OBJType <> OBJTYPE_MINERALES And ObjData(val(Arg2)).OBJType <> OBJTYPE_BARCOS And _
                                                                                                                                                                                                    ObjData(val(Arg2)).OBJType <> OBJTYPE_FLECHAS Then
                Call SendData(ToIndex, Userindex, 0, "||No esta permitido fabricar este tipo de objetos." & "´" & _
                                                     FontTypeNames.FONTTYPE_WARNING)
                Exit Sub
        ...
        Case "EXP"
            ...
            Else
                Call SendData(ToIndex, Userindex, 0, _
                              "||No esta permitido utilizar valores mayores a 5000. Su comando ha quedado en los logs del juego." _
                              & "´" & FontTypeNames.FONTTYPE_INFO)
                Exit Sub
        ...
        Case "BODY"
            '[GAU] agregamo UserList(UserIndex).Char.Botas
            Call ChangeUserChar(ToMap, 0, UserList(Tindex).Pos.Map, Tindex, val(Arg2), UserList( _
                                                                                       Tindex).Char.Head, UserList(Tindex).Char.Heading, UserList(Userindex).Char.WeaponAnim, _
        ...
        Case "HEAD"
            '[GAU] Agregamo UserList(UserIndex).Char.Botas
            Call ChangeUserChar(ToMap, 0, UserList(Tindex).Pos.Map, Tindex, UserList(Tindex).Char.Body, val( _
                                                                                                        Arg2), UserList(Tindex).Char.Heading, UserList(Userindex).Char.WeaponAnim, UserList( _
    ...
    If UCase$(Left$(rdata, 10)) = "/DONACION " Then
        ...
        Tindex = UserList(Userindex).flags.TargetUser
        'If UserList(Tindex).flags.Privilegios > 0 Then
            'Call SendData(ToIndex, Userindex, 0, "||No puedes dar Donaciones a un GM." & "´" & FontTypeNames.FONTTYPE_talk)
            'Exit Sub
        'End If
        If Tindex <= 0 Then
            Call SendData(ToIndex, Userindex, 0, "||Debes seleccionar al Jugador para Darle sus Creditos!" & "´" & FontTypeNames.FONTTYPE_talk)
            Exit Sub
        End If
        Call SendData(ToIndex, Userindex, 0, "||" & UserList(UserList(Userindex).flags.TargetUser).Name & " recibió " & rdata & " Creditos!!!" & "´" & FontTypeNames.FONTTYPE_talk)
        Call SendData(ToIndex, Tindex, 0, "||" & " Rebisite " & rdata & " Creditos, gracias por ayudar a World Of AO!!!" & "´" & FontTypeNames.FONTTYPE_talk)
        UserList(UserList(Userindex).flags.TargetUser).flags.Creditos = UserList(UserList(Userindex).flags.TargetUser).flags.Creditos + rdata
        
        Dim PuntosD As Integer
            PuntosD = UserList(UserList(Userindex).flags.TargetUser).flags.Creditos
            Call SendData(ToIndex, Tindex, 0, "J6" & PuntosD)
        Exit Sub
    ...
    If UCase$(rdata) = "/SOLOGM" Then
        ...
        If SoloGm = True Then Call SendData(ToIndex, Userindex, 0, "||Server cerrado al público." & "´" & _
                                                                   FontTypeNames.FONTTYPE_talk) Else Call SendData(ToIndex, Userindex, 0, "||Server abierto al público." _
                                                                                                                                          & "´" & FontTypeNames.FONTTYPE_talk)
        Exit Sub
    ...
    If UCase$(rdata) = "/ACTUALIZARWEB" Then
        If ActualizaWeb = 1 Then
            ActualizaWeb = 0
            Call SendData(ToIndex, Userindex, 0, "||Desactivado Actualizar Web en Clientes." & "´" & _
                                                 FontTypeNames.FONTTYPE_talk)
        Else
            ActualizaWeb = 1
            Call SendData(ToIndex, Userindex, 0, "||Activado Actualizar Web en Clientes." & "´" & _
                                                 FontTypeNames.FONTTYPE_talk)
...
ErrorHandler:
    Call LogError("TCP3. CadOri:" & CadenaOriginal & " Nom:" & UserList(Userindex).Name & "UI:" & Userindex & " N: " _
