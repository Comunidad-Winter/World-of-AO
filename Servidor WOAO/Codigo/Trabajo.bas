...
Public Sub DoPermanecerOculto(ByVal Userindex As Integer)
    ...
    If UserList(Userindex).Pos.Map > 199 And UserList(Userindex).Pos.Map < 208 Or UserList(Userindex).Pos.Map < 210 Then Exit Sub
...
Public Sub DoNavega(ByVal Userindex As Integer, ByRef Barco As ObjData)
    ...
    'PLUTO:2.4
    If UserList(Userindex).flags.Montura > 0 Or UserList(Userindex).flags.Angel > 0 Or UserList( _
       Userindex).flags.Morph > 0 Or UserList(Userindex).flags.Demonio > 0 Then Exit Sub
    ...
    If UserList(Userindex).Stats.UserSkills(Navegacion) / ModNave < Barco.MinSkill Then
        Call SendData(ToIndex, Userindex, 0, "||No tenes suficientes conocimientos para usar este barco." & "´" & _
                                             FontTypeNames.FONTTYPE_INFO)
        Call SendData(ToIndex, Userindex, 0, "||Para usar este barco necesitas " & Barco.MinSkill * ModNave & _
    ...
    Else
        ...
        'PLUTO:2.4
        If HayAgua(UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X + 1, UserList(Userindex).Pos.Y) And HayAgua( _
           UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList(Userindex).Pos.Y + 1) And HayAgua( _
           UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X, UserList(Userindex).Pos.Y - 1) And HayAgua( _
           UserList(Userindex).Pos.Map, UserList(Userindex).Pos.X - 1, UserList(Userindex).Pos.Y) Then
...
Public Sub DoLingotes(ByVal Userindex As Integer)
    ...
    Dim CantidadL As Integer
        ...
        If (UserList(Userindex).Invent.Object(UserList(Userindex).flags.TargetObjInvSlot).ObjIndex) = 192 Then
        CantidadL = Round(UserList(Userindex).Invent.Object(UserList(Userindex).flags.TargetObjInvSlot).Amount) / 5
        ElseIf (UserList(Userindex).Invent.Object(UserList(Userindex).flags.TargetObjInvSlot).ObjIndex) = 193 Then
        CantidadL = Round(UserList(Userindex).Invent.Object(UserList(Userindex).flags.TargetObjInvSlot).Amount) / 10
        ElseIf (UserList(Userindex).Invent.Object(UserList(Userindex).flags.TargetObjInvSlot).ObjIndex) = 194 Then
        CantidadL = Round(UserList(Userindex).Invent.Object(UserList(Userindex).flags.TargetObjInvSlot).Amount) / 20
        End If
    ...
    If RandomNumber(1, ObjData(UserList(Userindex).flags.TargetObjInvIndex).MinSkill) < 201 Then

        UserList(Userindex).Invent.Object(UserList(Userindex).flags.TargetObjInvSlot).Amount = UserList( _
                                                                                               Userindex).Invent.Object(UserList(Userindex).flags.TargetObjInvSlot).Amount - UserList(Userindex).Invent.Object(UserList(Userindex).flags.TargetObjInvSlot).Amount
...
Public Sub DoPescar(ByVal Userindex As Integer)
    ...
    If res < 11 Then
        ...
        'If res > 5 Then res = 1
        If UserList(Userindex).clase = "Pescador" Then
            MiObj.Amount = RandomNumber(1, CInt(UserList(Userindex).Stats.ELV / 6))
        Else
            MiObj.Amount = 1
...
Public Sub DoRobar(ByVal LadrOnIndex As Integer, ByVal VictimaIndex As Integer)
    ...
    If MapInfo(UserList(VictimaIndex).Pos.Map).Terreno = "ALDEA" Or MapInfo(UserList(VictimaIndex).Pos.Map).Terreno = _
       "TORNEO" Or MapInfo(UserList(VictimaIndex).Pos.Map).Terreno = "EVENTO" Then Exit Sub
